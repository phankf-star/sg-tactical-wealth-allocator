#!/usr/bin/env python3
"""
Global20Engine Base App Data-Source Governance Patch - CLEAN ACTIVE VERSION
Use this as the only active patch file:
  patch_base_app_data_source_governance.py

Purpose:
- Base app only; macro fetcher/workflows untouched.
- Prevent legacy/default/session PMI from driving production scoring/display.
- Include indentation repair in the same patch, so no separate repair workflow is needed.
"""
...
# PMI must read pack/override only
apply_sub(
...
# Macro Risk Score: remove pmi_value fallback
apply_sub(
...
# Empty-component fallback: remove PMI score from session/default
apply_sub(
...
# Pack-first PMI session bootstrap
apply_sub(
...
# PMI production card: no Seed fallback
apply_sub(
...
# PMI chart: macro history first, no simulated/default history
apply_sub(
...
# Live Market local score: pack PMI only, correctly indented inside render_market body
apply_sub(
...
# Indentation repair integrated here — no separate repair workflow needed.
# Known post-patch issue: rate_label/cards could become over-indented.
text2, n = re.subn(r'\n[ \t]+rate_label=rate_card_label\(index_label\)', '\nrate_label=rate_card_label(index_label)', text, count=1)
if n:
    text = text2; changes.append('repair rate_label indentation'); print('OK: repair rate_label indentation')
else:
    print('OK: rate_label indentation already clean or not found')
text2, n = re.subn(r'\n[ \t]+cards=\[\(', '\ncards=[(', text, count=1)
if n:
    text = text2; changes.append('repair cards indentation'); print('OK: repair cards indentation')
else:
    print('OK: cards indentation already clean or not found')

# Snapshot export uses pack PMI
