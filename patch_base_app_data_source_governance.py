#!/usr/bin/env python3
"""
Global20Engine Base App Data-Source Governance Patch v4
Use this as the active file: patch_base_app_data_source_governance.py

Works from either:
- restored pre-patch base app, or
- partially patched v2/v3 base app.

Goal: base-app only; macro fetcher untouched.
"""
...
backup = TARGET.with_suffix(TARGET.suffix + '.bak_data_source_governance_v4')
...
# PMI branch: do not use defaults
apply_sub(
...
# Macro Risk Score: remove pmi_value fallback from restored file or partial patched file
apply_sub(
...
# Session bootstrap: pack-first, safely indented at top-level app flow
apply_sub(
...
# PMI production card block: no Seed fallback
apply_sub(
...
# get_pmi_df: replace full function body only
apply_sub(
...
# Live Market local score: pack PMI only. Ensure indentation inside render_market expander.
apply_sub(
...
# Repair if previous patch left this block at top-level
apply_sub(
    r"\n_local_pmi_res = resolve_macro_value\(sel,'PMI'\)\nlatest_display = 0\.0 if not pmi_app else \(_local_pmi_res\.get\('value'\) if isinstance\(_local_pmi_res,dict\) and _local_pmi_res\.get\('value'\) is not None else float\('nan'\)\)\nlocal_score,local_alert,lvix,lcurve,lpmi,ldd,ltrend=calc_market_scores_by_asset\(sel,latest_display,dd,trend_below,vix,curve_spread\)",
    "\n        _local_pmi_res = resolve_macro_value(sel,'PMI')\n        latest_display = 0.0 if not pmi_app else (_local_pmi_res.get('value') if isinstance(_local_pmi_res,dict) and _local_pmi_res.get('value') is not None else float('nan'))\n        local_score,local_alert,lvix,lcurve,lpmi,ldd,ltrend=calc_market_scores_by_asset(sel,latest_display,dd,trend_below,vix,curve_spread)",
    'repair Live Market PMI score indentation'
)
