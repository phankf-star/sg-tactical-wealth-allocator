...
AMBER="#F59E0B"; AMBER_BG="#FEF3C7"; AMBER_TEXT="#92400E"; BLUE="#2563EB"; GREEN="#16A34A"; ORANGE="#F97316"; RED="#EF4444"; RED_BG="#FEE2E2"; SLATE="#64748B"; GREY_BORDER="#E5E7EB"; GREY_TEXT="#6B7280"
...
.decision-banner {{padding:18px 20px;border-radius:18px;background:{AMBER_BG};border:2px solid {AMBER};color:{AMBER_TEXT};margin:16px 0 18px 0;}}
.decision-chip {{display:inline-block;padding:7px 12px;border-radius:999px;background:#fff;border:1px solid {AMBER};margin:4px 6px 4px 0;font-weight:700;color:{AMBER_TEXT};}}
...
st.caption("Singapore wealth allocation dashboard with tactical allocation, market-specific PMI, live risk monitoring, staged deployment and crash-recovery analytics.")
...
ETF_UNIVERSE={"Straits Times Index (SG Value/REITs)":{"label":"🇸🇬 Singapore","etfs":[("Core exposure","SPDR STI ETF","ES3.SI","Broad STI exposure"),("Core alternative","Nikko AM STI ETF","G3B.SI","Alternative STI exposure")]},"Hang Seng Index (HK Cyclical/Beta)":{"label":"🇭🇰 Hong Kong","etfs":[("Core exposure","Tracker Fund of Hong Kong","2800.HK","Broad HSI exposure"),("Broad HSI ETF","iShares HSI ETF","3115.HK","Alternative HSI exposure"),("Higher beta satellite","iShares Hang Seng TECH ETF","3067.HK","Growth / tech sensitivity")]},"Nasdaq 100 (Tech Growth)":{"label":"🇺🇸 Nasdaq","etfs":[("Core exposure","Invesco QQQ","QQQ","Nasdaq 100 exposure"),("Lower-cost alternative","Invesco QQQM","QQQM","Nasdaq 100 lower-fee alternative")]},"S&P 500 (US Market Core)":{"label":"🇺🇸 S&P 500","etfs":[("Core exposure","SPDR S&P 500 ETF","SPY","Broad US large-cap exposure"),("Lower-cost core","Vanguard S&P 500 ETF","VOO","Low-cost S&P 500 exposure"),("Core alternative","iShares Core S&P 500 ETF","IVV","Broad S&P 500 exposure")]}}
...
def capital_breakdown(zone,deploy_amount,available_cash,available_srs,available_cpf):
    ...
    if zone=="INITIAL BUY": cash=min(deploy_amount,available_cash); reason="INITIAL BUY zone uses cash first; SRS/CPF-OA are preserved for deeper drawdowns."
    elif zone=="BUY": cash=min(deploy_amount,available_cash); rem=max(deploy_amount-cash,0); srs=min(rem,available_srs); reason="BUY zone uses cash first, then SRS if cash is insufficient. CPF-OA remains reserved."
    elif zone=="STRONG BUY": cash=min(deploy_amount,available_cash); rem=max(deploy_amount-cash,0); srs=min(rem,available_srs); rem=max(rem-srs,0); cpf=min(rem,available_cpf); reason="STRONG BUY zone can use cash, SRS and CPF-OA above preserved floor."
    else: reason="No deployment suggested under current drawdown zone."
...
def mini_pmi_bar_chart(df,title,subtitle):
    ...
    ymin=max(0,float(df["PMI"].min())-4); ymax=float(df["PMI"].max())+4
    fig.update_yaxes(range=[ymin,ymax]); fig.update_layout(height=250,margin=dict(l=10,r=10,t=58,b=10),title=f"{title}<br><sup>{subtitle}</sup>",plot_bgcolor="white",paper_bgcolor="white",showlegend=False,yaxis_title="PMI")
    st.plotly_chart(fig,use_container_width=True,config={"displayModeBar":False})

def html_card(title,value,sub,accent): return f"""<div class='col-card' style='--accent:{accent}'><div class='col-card-title'>{title}</div><div class='col-card-value'>{value}</div><div class='col-card-sub'>{sub}</div></div>"""
def preview_row(label,value,colour="#111827"): return f"<div class='preview-row'><span class='preview-label'>{label}</span><span class='preview-value' style='color:{colour}'>{value}</span></div>"
...
st.caption("Input and calculation engine: capital settings, drawdown reference, market-specific PMI, refresh controls and core metric outputs.")
...
with ref_col:
    ...
    drawdown_method=st.radio("Current drawdown reference",["Rolling 252D Peak","2Y Peak","3Y Peak","5Y Peak","All-Time High Peak"],index=0,horizontal=True,label_visibility="collapsed",help="Shorter references are tactical; longer references capture deeper market cycles.")
...
# PMI row with update button restored
...
with pmi1: pmi_proxy_label=st.selectbox("PMI Proxy Used", proxy_options, index=proxy_options.index(def_label) if def_label in proxy_options else 0)
with pmi2: pmi_region=st.text_input("PMI Region", value=pmi_proxy_default["region"] if pmi_proxy_label==def_label else pmi_proxy_label.replace(" PMI", ""))
with pmi3: pmi_source=st.text_input("PMI Source", value=pmi_proxy_default["source"] if pmi_proxy_label==def_label else "Economic calendar / manual source")
with pmi4:
...
with pmi5: pmi_month=st.text_input("PMI Month", value=st.session_state.get("latest_pmi_month","May 2026"))
with pmi6:
    st.markdown("<div style='height:27px'></div>", unsafe_allow_html=True)
    if st.button("🔄 Update PMI", use_container_width=True):
        st.session_state.latest_pmi_value=latest_pmi; st.session_state.latest_pmi_month=pmi_month; st.session_state.pmi_proxy_label=pmi_proxy_label; st.toast("PMI signal refreshed.", icon="🔄")
with pmi7:
    st.markdown("<div style='height:27px'></div>", unsafe_allow_html=True); manual_override=st.toggle("Manual", value=True, help="Manual override stays available as a fallback.")
...
# Risk score
vix_score=0 if vix is None else min(max((vix-15)*2,0),30); curve_score=10 if curve_spread is None else (20 if curve_spread<0 else 10 if curve_spread<0.5 else 0); pmi_score=0 if latest_pmi>=52 else 8 if latest_pmi>=50 else 16 if latest_pmi>=47 else 20; dd_score=min(abs(dd)*1.2,25); trend_score=15 if trend_below else 0; live_score=min(vix_score+curve_score+pmi_score+dd_score+trend_score,100)
...
with c1: st.metric(index_label,f"{close:,.0f}"); st.caption(f"{ticker} · Index Level")
...
# Tactical Decision Banner
funding_source="Cash First" if cash_deploy>0 else "No deployment"; decision_line="Deploy a small initial tranche only. Preserve SRS and CPF-OA for deeper drawdown zones." if deploy>0 else "No deployment now. Capital is preserved until a deployment trigger appears."; reason_line=f"Current market action is {zone}; risk regime is {alert}; PMI proxy used is {pmi_proxy_label}."
st.markdown(f"""
<div class='decision-banner'>
<h2 style='margin:0 0 8px 0'>🎯 Tactical Decision</h2>
<span class='decision-chip'>Verdict: {zone}</span><span class='decision-chip'>Deploy: S${deploy:,.0f}</span><span class='decision-chip'>Funding: {funding_source}</span><span class='decision-chip'>Risk: {alert}</span><span class='decision-chip'>Confidence: {conf_label}</span>
<p style='font-size:1.02rem;margin:10px 0 4px 0'><b>Decision:</b> {decision_line}</p>
<p style='font-size:0.92rem;margin:0'><b>Reason:</b> {reason_line}</p>
</div>
""", unsafe_allow_html=True)
...
with st.expander("💰 Suggested Deploy Basis & Capital Source", expanded=(deploy>0)):
    ...
    if options:
        st.markdown("#### 🎯 Suggested Investment Options"); st.caption("ETF-based educational options linked to the selected market. Not a personalised buy list."); st.dataframe(pd.DataFrame([{"Role":r,"Instrument":n,"Ticker":t,"Use case":u} for r,n,t,u in options]),use_container_width=True,hide_index=True)

with st.expander("📊 Signal Confidence Details", expanded=False):
    st.markdown(preview_row("Drawdown Signal","Active" if dd<=-8 else "Inactive",ORANGE if dd<=-8 else SLATE),unsafe_allow_html=True); st.markdown(preview_row("Macro Stress",alert,AMBER if alert=="WATCH" else ORANGE if alert=="WARNING" else RED if alert=="CRASH RISK" else GREEN),unsafe_allow_html=True); st.markdown(preview_row("Technical Trend","Weak" if trend_below else "Stable",BLUE),unsafe_allow_html=True); st.progress(conf_score/100, text=f"Signal Confidence: {conf_label} · {conf_score:.0f}/100")
with st.expander("🧱 Tranche Deployment Plan", expanded=(deploy>0)):
    if deploy>0:
        for label,amt,col,cap in [("Tranche 1 — Deploy now",deploy*0.5,AMBER,"Use 50% of suggested deploy while market is in the current buy zone."),("Tranche 2 — If drawdown deepens",deploy*0.25,ORANGE,"Deploy if selected index drawdown reaches the next trigger zone."),("Tranche 3 — If stabilisation appears",deploy*0.25,BLUE,"Deploy if risk score remains below WARNING and trend stabilises.")]: st.markdown(preview_row(label,f"S${amt:,.0f}",col),unsafe_allow_html=True); st.caption(cap)
...
with st.expander("🛑 Do Not Deploy Safeguards", expanded=False):
    ...
    for lab,active in checks: st.markdown(preview_row(lab,"Active" if active else "Inactive",RED if active else GREEN),unsafe_allow_html=True)
with st.expander("📡 Data Source & Freshness", expanded=False):
    st.markdown(preview_row("Market Data","Yahoo Finance",BLUE),unsafe_allow_html=True); st.markdown(preview_row("PMI Proxy",pmi_proxy_label,GREEN),unsafe_allow_html=True); st.markdown(preview_row("PMI Source",pmi_source,GREEN),unsafe_allow_html=True); st.markdown(preview_row("Last Refreshed",datetime.now().strftime('%d %b %Y %H:%M SGT'),SLATE),unsafe_allow_html=True)
with st.expander("📤 Tactical Snapshot Export", expanded=False):
    ...
    st.dataframe(snap,use_container_width=True,hide_index=True); st.download_button("⬇️ Export Tactical Snapshot CSV",snap.to_csv(index=False),file_name="tactical_snapshot.csv",mime="text/csv")

# Market Conditions & Live Risk Monitor
with st.expander("🌦️ MARKET CONDITIONS & LIVE RISK MONITOR",expanded=False):
    st.markdown("<h1 style='font-size:34px;margin-bottom:0'>🌦️ Market Conditions & Live Risk Monitor</h1><p class='small-note'>Live Risk Score = Global Macro Stress Overlay + Market-Specific PMI + Selected Market Technical Risk.</p>",unsafe_allow_html=True)
    ...
    with c: st.metric(pmi_proxy_label, f"{latest_pmi:.1f}")
    ...
    with right4:
        ...
        st.markdown(preview_row("VIX Score",f"{vix_score:.0f} / 30",AMBER),unsafe_allow_html=True); st.markdown(preview_row("Yield Curve Score",f"{curve_score:.0f} / 20",BLUE),unsafe_allow_html=True); st.markdown(preview_row(f"{pmi_proxy_label} Score",f"{pmi_score:.0f} / 20",GREEN),unsafe_allow_html=True); st.markdown(preview_row("Drawdown Score",f"{dd_score:.0f} / 25",ORANGE),unsafe_allow_html=True); st.markdown(preview_row("Trend Score",f"{trend_score:.0f} / 15",RED),unsafe_allow_html=True); st.markdown(f"<div class='alert-card {klass}'><b>Total Live Risk Score: {live_score:.0f} / 100 → {alert}</b></div>",unsafe_allow_html=True)
        with st.expander("ℹ️ How to read Live Risk Score", expanded=False): st.markdown(f"""**Live Risk Score = Global Macro Stress Overlay + Market-Specific PMI + Selected Market Technical Risk.**\n\nThis score is a **rules-based market stress indicator — not a crash prediction**.\n\n- **0–29 NORMAL:** low current stress signal.\n- **30–49 WATCH:** some caution signals are appearing.\n- **50–69 WARNING:** multiple indicators are deteriorating.\n- **70–100 CRASH RISK:** high stress regime.\n\nInputs currently include **VIX, US yield curve, {pmi_proxy_label}, selected index drawdown and selected index 200D trend**.""")
    with st.expander("📈 12M Trend Snapshot", expanded=False):
        ...
        vix_raw=hist("^VIX","2025-06-01"); vix_df=vix_raw[["Close"]].rename(columns={"Close":"VIX"}) if not vix_raw.empty else pd.DataFrame(); tnx_raw=hist("^TNX","2025-06-01"); irx_raw=hist("^IRX","2025-06-01"); tnx_df=tnx_raw[["Close"]].rename(columns={"Close":"TNX"}) if not tnx_raw.empty else pd.DataFrame(); irx_df=irx_raw[["Close"]].rename(columns={"Close":"IRX"}) if not irx_raw.empty else pd.DataFrame(); curve_df=pd.DataFrame()
...
with st.expander("🏆 CRASH & RECOVERY ANALYTICS",expanded=False):
    try:
        ...
        bt=ud.loc[pd.Timestamp(start):].copy(); bt["rm"]=bt.Close.rolling(252,min_periods=1).max(); bt["dd_pct"]=((bt.Close-bt.rm)/bt.rm)*100; cur=safe_float(bt.Close.iloc[-1]); ev=events(bt,thr,cur)
