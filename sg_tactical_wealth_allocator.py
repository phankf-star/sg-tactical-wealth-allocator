...
def current_dd(df,method):
    ...
    if method.startswith("Rolling"):
        days=252; label="Rolling 252D Peak"
    elif method.startswith("2Y"):
        days=504; label="2Y Peak"
    elif method.startswith("3Y"):
        days=756; label="3Y Peak"
    elif method.startswith("5Y"):
        days=1260; label="5Y Peak"
    else:
...
st.caption("Single top-level decision area: inputs, calculation result, risk regime, confidence and concise decision note.")
...
with pmi1: pmi_proxy_label=st.selectbox("PMI Proxy Used",PMI_PROXY_OPTIONS,index=PMI_PROXY_OPTIONS.index(def_label) if def_label in PMI_PROXY_OPTIONS else 0)
...
with pmi3: pmi_source=st.text_input("PMI Source",value=selected_actual["source"])
with pmi4:
    ...
    latest_pmi=st.number_input("Latest PMI",min_value=30.0,max_value=70.0,value=float(st.session_state.latest_pmi_value),step=.1,help="PMI is monthly and not intraday live data.")
...
with pmi6:
    ...
    if st.button("🔄 Update PMI",use_container_width=True):
        ...
        if latest:
            st.session_state.latest_pmi_value=float(latest["value"]); st.session_state.latest_pmi_month=latest["month"]; st.session_state.latest_pmi_source=latest["source"]; st.session_state.pmi_proxy_label=pmi_proxy_label
            st.toast(f"{pmi_proxy_label} updated to {latest['value']} for {latest['month']}.",icon="🔄"); st.rerun()
        else:
            st.session_state.latest_pmi_value=latest_pmi; st.session_state.latest_pmi_month=pmi_month; st.session_state.latest_pmi_source=pmi_source; st.session_state.pmi_proxy_label=pmi_proxy_label
            st.toast("PMI manual value saved.",icon="✏️")
with pmi7:
    st.markdown("<div style='height:27px'></div>",unsafe_allow_html=True); manual_override=st.toggle("Manual",value=True,help="Manual override stays available as fallback.")
latest_pmi=float(st.session_state.get("latest_pmi_value",latest_pmi)); pmi_month=st.session_state.get("latest_pmi_month",pmi_month); pmi_source=st.session_state.get("latest_pmi_source",pmi_source)
...
with st.expander("💰 Suggested Deploy Basis & Capital Source",expanded=(deploy>0)):
    ...
    if options:
        st.markdown("#### 🎯 Suggested Investment Options"); st.caption("ETF-based educational options linked to the selected market. Not a personalised buy list.")
...
with st.expander("🧭 Deployment Rule Ladder & Safeguards",expanded=False):
    ...
    with right:
        ...
        for lab,active in [("Live Risk Score > 70",live_score>70),("VIX > 35",vix is not None and vix>35),("PMI < 47",latest_pmi<47),("Emergency buffer breached",available_cash<=0)]:
...
with st.expander("📡 Audit Trail & Export",expanded=False):
    ...
    with left:
        ...
        for lab,val,col in [("Market Data","Yahoo Finance",BLUE),("PMI Proxy",pmi_proxy_label,GREEN),("PMI Value",f"{latest_pmi:.1f} · {pmi_month}",GREEN),("PMI Source",pmi_source,GREEN),("Last Refreshed",datetime.now().strftime('%d %b %Y %H:%M SGT'),SLATE)]: st.markdown(preview_row(lab,val,col),unsafe_allow_html=True)
    with right:
    ...
    st.markdown("#### 📤 Tactical Snapshot Export"); st.dataframe(snap,use_container_width=True,hide_index=True); st.download_button("⬇️ Export Tactical Snapshot CSV",snap.to_csv(index=False),file_name="tactical_snapshot.csv",mime="text/csv")
...
with st.expander("🏆 CRASH & RECOVERY ANALYTICS",expanded=False):
    st.info("Crash analytics module retained as a compact placeholder in this consolidated build. Expand prior full version if you want the full historical event explorer restored line-for-line.")
