# ─────────────────────────────────────────────────────────────────────────────
# Global20Engine v38af — Base App Data-Source Governance Audit patch; Live Market Trend Monitor
...
st.set_page_config(page_title='Crash Deployment Engine v38af', layout='wide', initial_sidebar_state='expanded')
st.markdown('''
<style>
.cde-compact-metric-grid{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:8px;margin:4px 0 12px;}
.cde-compact-metric{border:1px solid #E2E8F0;border-radius:12px;background:#FFFFFF;padding:9px 11px;box-shadow:0 4px 12px rgba(15,23,42,.04);min-height:58px;}
.cde-compact-metric span{display:block;font-size:.70rem;color:#64748B;font-weight:750;margin-bottom:4px;}
.cde-compact-metric b{display:block;font-size:1.02rem;line-height:1.08;color:#0F172A;font-weight:900;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.cde-table-head{font-size:.72rem;color:#64748B;font-weight:850;text-transform:uppercase;letter-spacing:.03em;margin:0 0 4px;}
.cde-alloc-budget{font-weight:850;color:#0F172A;background:#F8FAFC;border:1px solid #E2E8F0;border-radius:8px;padding:8px 10px;min-height:38px;}
@media(max-width:1100px){.cde-compact-metric-grid{grid-template-columns:repeat(3,minmax(0,1fr));}}
...
.cde-kpi-grid .cde-card{height:154px;min-height:154px;box-sizing:border-box;display:flex;flex-direction:column;justify-content:space-between;overflow:hidden}.cde-kpi-grid .cde-card-title{min-height:26px}.cde-kpi-grid .cde-main-value{line-height:1.05}.cde-nav-group-caption{font-size:11px;color:#64748b;font-weight:750;margin:2px 0 8px}.cde-dcc-title{font-size:19px;font-weight:950;color:#0f172a;margin:8px 0 2px}.cde-dcc-sub{font-size:12px;color:#475569;font-weight:800;margin-bottom:10px}.cde-dcc-section-title{font-size:14px;font-weight:950;color:#0f172a;margin:0 0 8px}.cde-dcc-mini{background:#f8fafc;border:1px solid #e2e8f0;border-radius:11px;padding:10px 11px;min-height:78px;display:flex;flex-direction:column;justify-content:center}.cde-dcc-mini-target{background:#ecfdf5;border-color:#bbf7d0;align-items:flex-start}.cde-dcc-mini span{display:block;font-size:10.5px;color:#64748b;font-weight:950;letter-spacing:.01em;line-height:1.25;margin-bottom:6px}.cde-dcc-mini b{display:block;font-size:17px;color:#0f172a;line-height:1.2}.cde-dcc-mini-target b{font-size:18px;color:#047857}.cde-dcc-mini-target .cde-market-flag{margin-right:4px;vertical-align:middle}.cde-dcc-split{border-top:1px solid #e2e8f0;margin:13px 0 12px}.cde-dcc-line{font-size:12.5px;color:#334155;font-weight:800;margin:5px 0}.cde-dcc-line b{color:#0f172a}.cde-dcc-main-amount{font-size:24px;font-weight:950;color:#0f172a;margin-bottom:8px}.cde-market-exposure{margin-top:6px;font-size:12px;color:#334155;font-weight:800}.cde-market-exposure-row{display:flex;justify-content:space-between;gap:8px;border-bottom:1px solid #eef2f7;padding:4px 0}.cde-capital-summary-row{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin:7px 0}.cde-capital-summary-row div{background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:8px}.cde-capital-summary-row b{display:block;font-size:16px;color:#059669;line-height:1.1}.cde-capital-summary-row span{font-size:10.5px;color:#64748b;font-weight:850}.cde-dcc-outer [data-testid="stVerticalBlockBorderWrapper"]{border-color:#d3dfec!important;border-radius:14px!important}.cde-dcc-outer div[data-testid="column"]{align-self:stretch}.cde-dcc-outer div[data-testid="stButton"] button{border-radius:999px;border-color:#d3dfec;font-size:12px;font-weight:850;color:#2563eb;background:#fff;padding:6px 12px}.cde-dcc-outer div[data-testid="stButton"]{margin-top:8px}


/* LP-R1 institutional hierarchy: Primary Deployment Action first; capital/exposure as compact control layer */
...
.cde-dcc-parent [data-testid="stVerticalBlockBorderWrapper"]{border:1px solid #bfdbfe!important;border-radius:18px!important;background:#fff!important;padding:12px 14px!important;box-sizing:border-box!important}.cde-dcc-parent div[data-testid="column"]{align-self:stretch!important}.cde-primary-action-card,.cde-watch-panel-card{height:236px!important;min-height:236px!important;max-height:236px!important;box-sizing:border-box!important;border:1px solid #d3dfec!important;border-radius:14px!important;background:#fff!important;padding:13px!important;overflow:hidden!important}.cde-primary-action-card .cde-dcc-section-title,.cde-watch-panel-card .cde-watch-title{font-size:15.5px!important;font-weight:950!important;line-height:1.2!important;color:#0f172a!important;margin:0 0 8px!important}.cde-watch-panel-card .cde-watch-sub{font-size:11px!important;color:#64748b!important;font-weight:800!important;margin:2px 0 10px!important}.cde-watch-panel-card .cde-watch-item{padding:9px 0!important}.cde-watch-panel-card .cde-watch-item b{font-size:12.8px!important}.cde-watch-panel-card .cde-watch-item span{font-size:12.6px!important}.cde-watch-panel-card .cde-watch-item em{font-size:12.6px!important;font-style:normal!important;font-weight:950!important}.cde-action-row{display:grid!important;grid-template-columns:1.35fr 1fr 1.05fr .9fr .9fr!important;gap:10px!important;margin-bottom:12px!important}.cde-action-mini{min-height:66px!important;padding:10px 11px!important;border-radius:11px!important;background:#f8fafc!important;border:1px solid #e2e8f0!important;display:flex!important;flex-direction:column!important;justify-content:center!important}.cde-action-mini.target{background:#ecfdf5!important;border-color:#bbf7d0!important}.cde-action-mini span{font-size:9.8px!important;color:#64748b!important;font-weight:850!important;line-height:1.1!important;margin-bottom:7px!important}.cde-action-mini b{font-size:15px!important;color:#0f172a!important;line-height:1.1!important;white-space:nowrap!important}.cde-action-mini.target b{font-size:18px!important;color:#047857!important;display:flex!important;align-items:center!important;gap:6px!important}.cde-action-mini.target .cde-flag-circle{width:17px!important;height:17px!important;min-width:17px!important;font-size:9px!important;margin:0!important}.cde-next-action-grid{display:grid!important;grid-template-columns:0.42fr 0.58fr!important;gap:12px!important;border-top:1px solid #e2e8f0!important;padding-top:11px!important}.cde-next-amount{font-size:24px!important;font-weight:950!important;color:#0f172a!important;line-height:1.05!important;margin:4px 0 7px!important}.cde-action-line{font-size:11.5px!important;color:#334155!important;font-weight:800!important;margin:3px 0!important;line-height:1.25!important}.cde-action-status-box{background:#f8fafc!important;border:1px solid #e2e8f0!important;border-radius:10px!important;padding:9px 10px!important;height:82px!important}.cde-action-status-box b{display:block!important;color:#059669!important;font-size:18px!important;margin-bottom:4px!important}.cde-action-status-box span{font-size:11px!important;color:#64748b!important;font-weight:850!important}.cde-control-card{border:1px solid #d3dfec!important;border-radius:14px!important;background:#fff!important;padding:14px!important;margin-bottom:10px!important}.cde-control-title{font-size:14px!important;font-weight:950!important;color:#0f172a!important;margin-bottom:8px!important}.cde-control-row{display:grid!important;grid-template-columns:1fr auto auto!important;gap:8px!important;align-items:center!important;border-bottom:1px solid #eef2f7!important;padding:4px 0!important}.cde-control-row span{font-size:11px!important;color:#64748b!important;font-weight:850!important}.cde-control-row b{font-size:12px!important;color:#059669!important;font-weight:950!important}.cde-control-row em{font-style:normal!important;font-size:11px!important;color:#dc2626!important;font-weight:950!important}.cde-control-exposure{font-size:11px!important;color:#334155!important;font-weight:850!important;margin-top:8px!important}.cde-control-exposure .cde-market-exposure-row{padding:3px 0!important;border-bottom:1px solid #eef2f7!important}.cde-control-card div[data-testid="stButton"] button,.cde-dcc-parent div[data-testid="stButton"] button{border-radius:999px!important;border-color:#d3dfec!important;font-size:11px!important;font-weight:850!important;color:#2563eb!important;background:#fff!important;padding:4px 9px!important}.cde-dcc-title{font-size:18px!important}.cde-dcc-sub{font-size:11.5px!important;color:#475569!important;margin-bottom:8px!important}



/* CDE LP visual refinement patch 2026-07-06 */
...
.cde-watch-edge-card{margin-top:4px!important;padding:8px 9px!important;border:1px solid #dbeafe!important;border-radius:12px!important;background:#f8fbff!important;}
.cde-watch-edge-title{font-size:11px!important;font-weight:950!important;color:#0f172a!important;line-height:1.15!important;margin-bottom:6px!important;}
.cde-watch-edge-grid{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:6px!important;}
.cde-watch-edge-grid div{background:#fff!important;border:1px solid #e2e8f0!important;border-radius:9px!important;padding:5px 6px!important;text-align:center!important;}
.cde-watch-edge-grid b{display:block!important;font-size:13px!important;font-weight:950!important;color:#059669!important;line-height:1.05!important;}
.cde-watch-edge-grid span{display:block!important;font-size:8.5px!important;font-weight:800!important;color:#64748b!important;line-height:1.1!important;margin-top:2px!important;}
.cde-next-action-grid{grid-template-columns:minmax(210px,1fr) minmax(250px,1.12fr)!important;gap:12px!important;align-items:stretch!important;}
.cde-next-deployment-hero{border:1px solid #bfdbfe!important;border-radius:14px!important;background:linear-gradient(135deg,#eff6ff 0%,#f0fdf4 100%)!important;padding:13px 15px!important;box-shadow:inset 4px 0 0 #2563eb,0 1px 2px rgba(15,23,42,.04)!important;min-height:82px!important;box-sizing:border-box!important;}
...
/* CDE LP restore 2026-07-06: Primary left 71%, Watchlist right 29%; remove forced position hack. */
.cde-dcc-parent{width:100%!important;overflow:visible!important;}
.cde-command-grid{display:grid!important;grid-template-columns:minmax(0,71fr) minmax(280px,29fr)!important;grid-template-areas:none!important;gap:16px!important;align-items:stretch!important;width:100%!important;box-sizing:border-box!important;}
.cde-command-grid>.cde-primary-action-card{order:0!important;grid-column:auto!important;grid-row:auto!important;grid-area:auto!important;min-width:0!important;width:100%!important;max-width:none!important;overflow:visible!important;}
.cde-command-grid>.cde-watch-panel-card{order:0!important;grid-column:auto!important;grid-row:auto!important;grid-area:auto!important;min-width:0!important;width:100%!important;max-width:none!important;overflow:visible!important;}
.cde-primary-action-card{display:flex!important;flex-direction:column!important;}
.cde-action-row{position:relative!important;z-index:2!important;margin-bottom:14px!important;}
.cde-next-action-grid{position:relative!important;z-index:1!important;margin-top:12px!important;clear:both!important;grid-template-columns:minmax(250px,1fr) minmax(260px,1.25fr)!important;gap:12px!important;align-items:stretch!important;}
.cde-next-deployment-hero{position:relative!important;z-index:1!important;min-height:96px!important;padding:13px 16px!important;}
.cde-next-deployment-hero .cde-next-amount{font-size:clamp(28px,2.65vw,36px)!important;line-height:1.02!important;word-break:normal!important;overflow-wrap:normal!important;}
.cde-action-status-box{height:auto!important;min-height:96px!important;overflow:visible!important;white-space:normal!important;}
.cde-action-status-box span{white-space:normal!important;line-height:1.45!important;}
@media (max-width:900px){.cde-command-grid{grid-template-columns:1fr!important;}.cde-next-action-grid{grid-template-columns:1fr!important;}}



/* CDE LP final correction 2026-07-06: Primary left 71%, Watchlist right 29%, softer Next Deployment. */
.cde-dcc-parent{width:100%!important;overflow:visible!important;}
.cde-command-grid{
  display:grid!important;
  grid-template-columns:minmax(0,71fr) minmax(280px,29fr)!important;
  grid-template-areas:"primary watch"!important;
  gap:16px!important;
  align-items:stretch!important;
  width:100%!important;
...
.cde-next-action-grid{
  position:relative!important;
  z-index:1!important;
  margin-top:8px!important;
  clear:both!important;
  display:grid!important;
  grid-template-columns:minmax(240px,.95fr) minmax(260px,1.25fr)!important;
  gap:12px!important;
  align-items:stretch!important;
}
.cde-next-deployment-hero{
  position:relative!important;
  z-index:1!important;
...
.cde-next-deployment-hero .cde-dcc-section-title{
  font-size:13.5px!important;
  line-height:1.1!important;
  margin-bottom:5px!important;
}
.cde-next-deployment-hero .cde-next-amount{
...
.cde-next-deployment-hero .cde-action-line{
  margin-top:5px!important;
  font-size:11px!important;
  line-height:1.2!important;
}
.cde-action-status-box{
...
# ─────────────────────────────────────────────────────────────────────────────
# LP-R1 Responsive Executive Dashboard System - dynamic screen adaptation
# Desktop/laptop: bounded command board; tablet/mobile: progressive stacking.
# ─────────────────────────────────────────────────────────────────────────────
st.markdown("""
<style>
.block-container{max-width:1360px !important;margin-left:auto !important;margin-right:auto !important;padding-left:1.15rem !important;padding-right:1.15rem !important;}
.cde-kpi-grid{display:grid !important;grid-template-columns:repeat(5,minmax(0,1fr)) !important;gap:14px !important;align-items:stretch !important;}
...
.cde-command-grid .cde-watch-panel-card{order:1 !important;}.cde-command-grid .cde-primary-action-card{order:2 !important;}
.cde-action-row{display:grid !important;grid-template-columns:repeat(5,minmax(0,1fr)) !important;gap:8px !important;}
.cde-next-action-grid{display:grid !important;grid-template-columns:minmax(170px,.9fr) minmax(220px,1.1fr) !important;gap:12px !important;align-items:stretch !important;}
...
def fetch_yahoo_us_10y():
    ...
    try:
        ...
        if df is None or df.empty:
            ...
            return None,'N/A'
        raw_val=safe_float(df.Close.iloc[-1])
        val=raw_val/10.0 if raw_val>20 else raw_val
...
def _test_yahoo_tnx():
    try:
        ...
        raw=safe_float(df.Close.iloc[-1])
        v=raw/10.0 if raw>20 else raw
...
def _flatten_json_dicts(obj):
    rows=[]
    if isinstance(obj,dict):
        rows.append(obj)
        for v in obj.values(): rows.extend(_flatten_json_dicts(v))
    elif isinstance(obj,list):
        for v in obj: rows.extend(_flatten_json_dicts(v))
    return rows
...
def _parse_sg_sora_from_redistributor_html(txt, source_name):
    """Parse dated SORA values from public live pages that republish MAS SORA.
    Do not parse undated summary/change values; require an explicit date near the number.
    """
    if not txt:
        return None
    clean=re.sub(r'<script[\s\S]*?</script>|<style[\s\S]*?</style>',' ',txt,flags=re.I)
    clean=re.sub(r'<[^>]+>',' ',clean)
    clean=re.sub(r'\s+',' ',clean).strip()
    candidates=[]
    months='Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec'
    for m in re.finditer(r'(?:(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun),\s*)?(\d{1,2}\s+(?:'+months+r')\s+20\d{2})\s+(-?\d+(?:\.\d+)?)\s*%', clean, flags=re.I):
        dt=pd.to_datetime(m.group(1),errors='coerce'); val=_clean_number(m.group(2))
        if pd.notna(dt) and val is not None and 0.05 <= val <= 25:
            candidates.append((pd.Timestamp(dt),val,source_name))
    for m in re.finditer(r'(20\d{2}[-/]\d{1,2}[-/]\d{1,2})\s+(-?\d+(?:\.\d+)?)\s*%', clean, flags=re.I):
        dt=pd.to_datetime(m.group(1),errors='coerce'); val=_clean_number(m.group(2))
        if pd.notna(dt) and val is not None and 0.05 <= val <= 25:
            candidates.append((pd.Timestamp(dt),val,source_name))
    for m in re.finditer(r'\bSORA\b\s+(-?\d+(?:\.\d+)?)\s*%\s+As of\s+(\d{1,2}\s+(?:'+months+r')\s+20\d{2})', clean, flags=re.I):
        val=_clean_number(m.group(1)); dt=pd.to_datetime(m.group(2),errors='coerce')
        if pd.notna(dt) and val is not None and 0.05 <= val <= 25:
            candidates.append((pd.Timestamp(dt),val,source_name))
    if not candidates:
        return None
    candidates=sorted(candidates,key=lambda x:(x[0], x[1]))
    dt,val,src=candidates[-1]
    return val,dt.strftime('%d %b %Y'),src,len(candidates)
...
def fetch_mas_sora_rate():
    """SG Rates live adapter: redistributor-only path.

    v38z decision: remove legacy MAS secure/eservices datastore/API endpoints from the active fetch path because diagnostics show those routes are not reliable in this runtime.
    The live path uses public SORA pages that explicitly republish MAS SORA values. Source transparency is retained in the tooltip/diagnostics.
    """
    adapter='SG SORA live redistributor only'
    sources=[
        ('MacroMicro MAS SORA redistributor','https://en.macromicro.me/series/6324/singapore-overnight-rate-average-sora'),
        ('TradingEconomics MAS SORA redistributor','https://tradingeconomics.com/singapore/interest-rate'),
        ('StraitsData MAS SORA redistributor','https://straitsdata.com/finance/mas'),
        ('Cashew SG SORA redistributor','https://www.cashew.sg/interest-rates/sora'),
    ]
    last=''
    for src_name,url in sources:
        txt,err,row=_request_text_custom(url,src_name,capture_global=True,extra_headers={'Accept':'text/html,application/xhtml+xml,text/plain,*/*','Accept-Encoding':'identity'})
        if not txt:
            last=err; continue
        parsed=_parse_sg_sora_from_redistributor_html(txt,src_name)
        if parsed:
            val,dt,src,n=parsed
            _diag(adapter,url,True,n,'Dated SG SORA value parsed',f'{dt}={val}',f'Legacy MAS secure/eservices datastore endpoints removed from active fetch path. Source: {src_name}')
            return val,dt,src,'Legacy MAS secure/eservices datastore endpoints removed from active fetch path; live redistributor source used.'
        last=f'{src_name} reached but no dated SORA value parsed'
    return None,'N/A','SG SORA live redistributor only',last or 'No redistributor returned a dated SG SORA value'
...
def fetch_boj_overnight_call_rate():
    """JP Rates primary adapter: BOJ FM01 STRDCLUCON CSV parser."""
    adapter='BOJ FM01 STRDCLUCON CSV parser'
    ...
    for code in ['STRDCLUCON','STRDCLUCON@D','FM01.STRDCLUCON','FM01.STRDCLUCON@D']:
        ...
        if parsed:
            val,dt,n=parsed
            _diag(adapter,url,True,n,f'FM01 CSV parsed via code {code}',f'{dt}={val}','')
...
def rate_basis_text(market):
    ...
    if market=='STI': return 'Basis: Singapore-dollar interest-rate environment. Preferred basis: SG SORA live redistributor pages that explicitly cite MAS SORA; legacy MAS secure/eservices datastore endpoints removed from active fetch path.'
    ...
    if market=='Nikkei 225':
        return 'Basis: Japan overnight money-market rate. Preferred source: BOJ FM01 STRDCLUCON CSV parser.'
...
def resolve_macro_value(market,indicator):
    ...
    if indicator=='Rates':
        ...
        if market=='STI':
            ...
            if val is not None: return _source_result(val,f'{val:.2f}%',f'Live source · {src} · {dt}','Live source',dt,err)
        ...
        if market=='Nikkei 225':
            ...
            if val is not None: return _source_result(val,f'{val:.2f}%',f'Official API · {src} · {dt}','Official API',dt,err)
            return _awaiting_live('BOJ FM01 STRDCLUCON CSV parser', err or 'BOJ JP rates adapter returned no usable value')
...
def deploy_box_colours_for_zone(zone_name):
    if zone_name == 'HOLD / NO DEPLOYMENT':
        return '#F1F5F9', '#CBD5E1'
    if zone_name == 'INITIAL BUY':
...
def deploy_rule(dd):
    # Cumulative deployment of available investible capital / cash.
...
def capital_breakdown(zone, deploy_amount, available_cash, available_srs, available_cpf):
    ...
    reason = (
        ...
        'MAX CRISIS BUY zone deploys 100% of investible cash after excluding safeguards.'
    )
    ...
    if isinstance(etf_master,dict):
        for key,rec in etf_master.items():
            ...
            rows.append({
                ...
                'AUM':_etf_num(rec.get('aum') or rec.get('aum_usd') or rec.get('assets_under_management') or rec.get('net_assets'),0.0),
                'AUM Currency':rec.get('aum_currency') or rec.get('aum_ccy') or rec.get('currency') or '',
                'Expense Ratio':_etf_num(rec.get('expense_ratio') or rec.get('expense_ratio_pct') or rec.get('ter') or rec.get('cost_pct'),0.0),
                'Dividend Yield':_etf_num(rec.get('dividend_yield') or rec.get('dividend_yield_pct') or rec.get('yield_pct'),0.0),
...
def _etf_eligibility_symbol(value):
    raw=str(value or '').strip(); low=raw.lower()
    if raw=='✓' or 'eligible ✓' in low or low=='eligible': return '✓'
    if raw in ['✕','x','X'] or 'not eligible' in low: return '✕'
    return 'ⓘ'

def _etf_eligibility_html(symbol, field_name):
    if symbol == '✓': txt=f'{field_name}: marked eligible in ETF master. Verify before using this funding source.'
    elif symbol == '✕': txt=f'{field_name}: marked not eligible in ETF master. Verify before using this funding source.'
    else: txt=f'{field_name}: eligibility not confirmed. Verify with broker / platform / CPF-SRS provider.'
    return f'<span title="{hesc(txt)}" class="etf-row-text" style="display:inline-block;min-width:14px;text-align:center;font-weight:800;font-size:12px;line-height:1;cursor:help;white-space:nowrap;">{hesc(symbol)}</span>'

def _etf_eligibility_help():
    return _etf_mini_tip('CPF-OA / SRS Eligibility', [('✓','Eligible'),('✕','Not eligible'),('ⓘ','Unknown / verify')], 'Verify with broker / platform / CPF-SRS provider before using this funding source.')

def _etf_header_html(label):
    return f'<div class="cde-mo-head etf-table-head" style="white-space:nowrap;font-size:13px;line-height:1.15;font-weight:900;color:#111827;margin-bottom:0.22rem;">{label}</div>'
...
def _etf_aum_display(row):
    try:
        ...
        return f'{c} {txt}'.strip()
    except Exception: return '—'

def _etf_display_cost(value):
    try:
...
def render_etf_add_entry_rows(etf_df, market_name, market_ccy, key_prefix='etf_add_entry_rows', allow_promote=True, rich=False):
    ...
    if etf_df is None or etf_df.empty:
        ...
        return None
    st.markdown('''<style>
      .etf-row-text{font-size:13px;line-height:1.12;margin:0!important;padding:0!important;}
      .etf-table-head{font-size:13px!important;font-weight:900!important;line-height:1.15!important;}
      .etf-mini-tip{transform:scale(.86);transform-origin:center;}
      div[data-testid="stHorizontalBlock"]{margin-bottom:0.24rem!important;gap:0.42rem!important;align-items:center!important;}
      div[data-testid="stVerticalBlock"]{gap:0.14rem!important;}
    ...
    elig=_etf_eligibility_help()
    if rich:
        widths=[.42,1.70,.66,.52,.44,.42,.42,.54,.64,.54,.48,.48,.48,.48,.58]
        headers=['Rank','Instrument','Ticker','Role','Ccy',f'CPF {elig}',f'SRS {elig}','Price','AUM','Cost %','1Y','3Y','5Y','Fit','Promote']
    elif allow_promote:
        widths=[.42,1.80,.72,.56,.48,.44,.44,.58,.64,1.12,.60]
        headers=['Rank','Instrument','Ticker','Role','Ccy',f'CPF {elig}',f'SRS {elig}','Price','Fit','Use Case','Promote']
    else:
        widths=[.42,1.96,.76,.56,.48,.44,.44,.58,.64,1.24]
        headers=['Rank','Instrument','Ticker','Role','Ccy',f'CPF {elig}',f'SRS {elig}','Price','Fit','Use Case']
    for col,label in zip(st.columns(widths),headers): col.markdown(_etf_header_html(label), unsafe_allow_html=True)
    for pos,(_,row) in enumerate(etf_df.iterrows(),1):
        ...
        if rich:
            ...
            cols[9].caption(use_case); promote_col_idx=10 if allow_promote else None
        if allow_promote and _etf_display_text(row.get('Promote'),'')=='Request':
            if cols[promote_col_idx].checkbox('',key=f'{key_prefix}_promote_{market_name}_{ticker}_{pos}',help='Request owner-gated promotion to Platform Default ETF.'): selected_promotion_row=row.to_dict()
        elif allow_promote: cols[promote_col_idx].write('')
...
def _capital_settings_payload(base_currency=None,total_capital=None,manual_allocated=None,market_alloc=None):
    return _normalise_capital_settings({
        ...
        'investible_srs_input': float(st.session_state.get('investible_srs_input',0.0) or 0),
        'include_cpf_oa_sti': bool(st.session_state.get('include_cpf_oa_sti',False)),
        'cpf_oa_balance_input': float(st.session_state.get('cpf_oa_balance_input',0.0) or 0),
        'preserve_cpf_floor_input': bool(st.session_state.get('preserve_cpf_floor_input',True)),
    })
...
def _capital_source_breakdown(base_currency=None):
    """Return included portfolio funding capacity by source.

    Base cash is always part of portfolio capital. SRS / CPF-OA are included only
    when the base currency is SGD and the relevant inclusion toggle is on. CPF-OA
    available capacity respects the optional S$20k CPF-OA floor.
    """
    _init_capital_settings()
    base_currency=str(base_currency or st.session_state.get('base_capital_currency','SGD')).upper()
    base_cash=max(float(st.session_state.get('total_investible_capital_input',0.0) or 0.0),0.0)
    include_srs=bool(st.session_state.get('include_srs_sti',False)) if base_currency=='SGD' else False
    include_cpf=bool(st.session_state.get('include_cpf_oa_sti',False)) if base_currency=='SGD' else False
    srs=max(float(st.session_state.get('investible_srs_input',0.0) or 0.0),0.0) if include_srs else 0.0
    cpf_raw=max(float(st.session_state.get('cpf_oa_balance_input',0.0) or 0.0),0.0) if include_cpf else 0.0
    preserve=bool(st.session_state.get('preserve_cpf_floor_input',True)) if include_cpf else False
    cpf_floor=20000.0 if (include_cpf and preserve) else 0.0
    cpf_available=max(cpf_raw-cpf_floor,0.0) if include_cpf else 0.0
    total=base_cash+srs+cpf_available
    parts=['Cash']
    if include_srs: parts.append('SRS')
    if include_cpf: parts.append('CPF-OA')
    return {'base_currency':base_currency,'cash':base_cash,'srs':srs,'cpf_raw':cpf_raw,'cpf_floor':cpf_floor,'cpf_available':cpf_available,'total':total,'include_srs':include_srs,'include_cpf':include_cpf,'funding_profile':' + '.join(parts)}

def _funding_breakdown_pairs(required=0.0, market_available=0.0, portfolio_available=0.0, coverage_ratio=None, base_currency=None):
    cap=_capital_source_breakdown(base_currency)
    pairs=[('Cash / Base Capital', fmt_sgd(cap['cash']))]
    if cap['include_srs']:
        pairs.append(('SRS included', fmt_sgd(cap['srs'])))
    if cap['include_cpf']:
        cpf_label='CPF-OA after floor' if cap['cpf_floor']>0 else 'CPF-OA included'
        pairs.append((cpf_label, fmt_sgd(cap['cpf_available'])))
        if cap['cpf_floor']>0:
            pairs.append(('CPF-OA safeguard floor', fmt_sgd(cap['cpf_floor'])))
    pairs.extend([('Total included funding capacity', fmt_sgd(cap['total'])),('Required deployment', fmt_sgd(required)),('Market available budget', fmt_sgd(market_available)),('Portfolio cash available', fmt_sgd(portfolio_available))])
    if coverage_ratio is not None:
        pairs.append(('Coverage ratio', 'N/A' if required<=0 else f'{float(coverage_ratio):.1f}x'))
    return pairs

def _normalise_funding_source_series(series):
    s=series.astype(str).str.strip() if hasattr(series,'astype') else pd.Series(dtype=str)
...
def _funding_readiness_matrix(required, market_budget, market_actual, market_pending, portfolio_dry_powder, market_name='Market'):
    ...
    if not budget_assigned:
        ...
        code,label,cls='funding_action','⚠ Funding Action','insufficient'; detail=f'Portfolio funding shortfall: {fmt_sgd(portfolio_dry_powder)} cash vs {fmt_sgd(required)} required.'
    elif pending_blocks:
        ...
        code,label,cls='ready','✅ Ready','ready'; detail=f'{fmt_sgd(market_available)} {market_name} allocation available; {fmt_sgd(portfolio_dry_powder)} portfolio cash available.'
...
def _market_budget_amount(market,total_cap=None):
    if total_cap is None:
        total_cap=float(_capital_source_breakdown().get('total',0.0) or 0.0)
...
def render_market_allocation_budget_ui(base_symbol,total_input,compact=False):
    ...
    st.caption('Select interested markets and assign % of total portfolio capital to each market. Total assigned should not exceed 100%; unassigned capital remains portfolio cash.')
    alloc=dict(st.session_state.get('market_allocation_budget_pct',{}) or {})
    base_currency=st.session_state.get('base_capital_currency','SGD')
    actual_by_market, _actual_total = _executed_base_exposure_by_market(base_currency)
    alloc_rows=[]
    header_cols=st.columns([.16,.9,.48,.62,.62,.62])
    for c,label in zip(header_cols,['','Market','Allocation %','Market Budget','Actual Deployed','Available Budget']):
        c.markdown(f'<div class="cde-table-head">{label}</div>',unsafe_allow_html=True)
    for mk in CDE_ALLOCATABLE_MARKETS:
        existing=float(alloc.get(mk,0.0) or 0.0)
        row_cols=st.columns([.16,.9,.48,.62,.62,.62])
        enabled=row_cols[0].checkbox('',value=existing>0,key='alloc_enable_'+re.sub(r'[^A-Za-z0-9]+','_',mk))
        row_cols[1].markdown(f'**{hesc(mk)}**',unsafe_allow_html=True)
        ...
        if not enabled:
            pct=0.0
        alloc[mk]=float(pct)
        budget=float(total_input)*float(pct)/100
        actual=max(float(actual_by_market.get(mk,0.0)),0.0) if isinstance(actual_by_market,dict) else 0.0
        available=max(budget-actual,0.0)
        row_cols[3].markdown(f'<div class="cde-alloc-budget">{base_symbol}{budget:,.2f}</div>',unsafe_allow_html=True)
        row_cols[4].markdown(f'<div class="cde-alloc-budget">{base_symbol}{actual:,.2f}</div>',unsafe_allow_html=True)
        row_cols[5].markdown(f'<div class="cde-alloc-budget">{base_symbol}{available:,.2f}</div>',unsafe_allow_html=True)
    ...
    a2.metric('Unassigned Cash',f'{unassigned:.0f}%')
    a3.metric('Unassigned Amount',f'{base_symbol}{float(total_input)*unassigned/100:,.2f}')
    if total_assigned>100.0:
        st.error('Total assigned allocation exceeds 100%. Please reduce one or more market allocations.')
    if not compact:
        with st.expander('Allocation Audit Table', expanded=False):
...
if st.session_state.asset_group_selection not in ASSET_GROUPS:
    st.session_state.asset_group_selection = 'Market / Equity Index'
if 'selected_market_name' not in st.session_state:
    default_items = ASSET_GROUPS[st.session_state.asset_group_selection]
    st.session_state.selected_market_name = 'STI' if 'STI' in default_items else default_items[0]
_init_capital_settings()
with st.sidebar:
    logo_asset = Path('assets/cde_logo.png')
    ...
    st.markdown(f'''<div style="font-size:11px;font-weight:700;color:#64748B;line-height:1.1;margin:5px 0 8px 2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">Market data: {datetime.now().strftime('%d %b %Y %H:%M SGT')}</div>''', unsafe_allow_html=True)
    st.markdown('''<style>
    section[data-testid="stSidebar"] .stButton>button{min-height:25px!important;padding:2px 8px!important;font-size:12px!important;margin:0!important}
    section[data-testid="stSidebar"] label{font-size:11.5px!important;margin-bottom:0!important}
    section[data-testid="stSidebar"] div[data-testid="stSelectbox"]{margin-bottom:2px!important}
    .cde-subpanel{padding:3px 0 4px 8px!important;margin:0 0 5px 8px!important;border-left:1px solid #E2E8F0!important}
    .cde-holdings-table{width:100%;border-collapse:collapse;font-size:12px;line-height:1.2}.cde-holdings-table th{background:#F8FAFC;color:#334155;font-weight:900;border-bottom:1px solid #E2E8F0;padding:6px 7px;text-align:left}.cde-holdings-table td{border-bottom:1px solid #EEF2F7;padding:6px 7px;color:#0F172A;font-weight:750}.cde-holdings-table td.num{text-align:right;font-variant-numeric:tabular-nums}.cde-holdings-table tfoot td{background:#F8FAFC;font-weight:950;border-top:1px solid #CBD5E1}.cde-status-na{color:#B45309;font-weight:900}.cde-status-live{color:#059669;font-weight:900}
    .cde-mini-plan{display:grid;grid-template-columns:1fr auto;gap:6px 12px;font-size:12.5px;line-height:1.15;margin-top:6px}.cde-mini-plan span{color:#475569}.cde-mini-plan b{font-weight:950}.cde-mini-plan .green{color:#059669}.cde-mini-plan .orange{color:#F97316}.cde-mini-plan .blue{color:#2563EB}.cde-mini-plan .red{color:#DC2626}
    </style>''', unsafe_allow_html=True)
    ...
    def _nav_button(label):
        if st.session_state.active_section == label:
            ...
            return False
        return st.button(label, use_container_width=True, key=_nav_key(label))
    ...
    with st.expander('Dashboard', expanded=True):
        ...
        if st.session_state.active_section == '▣ Market Deep Dive':
            ...
            if current_ag not in ag_options:
                current_ag='Market / Equity Index'; st.session_state.asset_group_selection=current_ag
            asset_group=st.selectbox('Asset',ag_options,index=ag_options.index(current_ag),key='sidebar_mdd_asset_group')
            ...
            if current_sel not in group_items:
                current_sel=default_item; st.session_state.selected_market_name=current_sel
            sel=st.selectbox('Market' if asset_group=='Market / Equity Index' else 'Asset',group_items,index=group_items.index(current_sel),key='sidebar_mdd_selected_market')
            st.session_state.selected_market_name=sel
            st.markdown('</div>', unsafe_allow_html=True)
        else:
            asset_group=st.session_state.get('asset_group_selection','Market / Equity Index')
    ...
    with st.expander('Capital Management', expanded=True):
        if _nav_button('💰 Capital Settings'):
            st.session_state.active_section='💰 Capital Settings'
        if _nav_button('🧭 Philosophy, Strategy & Allocation Rules'):
            st.session_state.active_section='🧭 Philosophy, Strategy & Allocation Rules'

    cap_sources=_capital_source_breakdown(base_currency_code)
    total_investible_state=float(cap_sources.get('total',0.0) or 0.0)
    funding_profile=cap_sources.get('funding_profile','Cash')
    cash_balance=float(cap_sources.get('cash',0.0) or 0.0)
    srs_balance=float(cap_sources.get('srs',0.0) or 0.0)
    cpf_oa_balance=float(cap_sources.get('cpf_raw',0.0) or 0.0)
    preserve_cpf=bool(cap_sources.get('cpf_floor',0.0)>0)
...
available_cash=max(cash_balance,0); available_srs=srs_balance; available_cpf=max(cpf_oa_balance-(20000 if preserve_cpf else 0),0); total_available=available_cash+available_srs+available_cpf
# Individual-market Suggested Deploy is based on selected market allocation budget, not total portfolio capital.
# This early reader cannot rely on later-defined Trade Journal helpers, so it reads the trade log directly.
def _early_selected_market_executed_base(market, base_currency):
    try:
        trade_path=Path('cde_trade_log.csv')
        if not trade_path.exists(): return 0.0
        df=pd.read_csv(trade_path)
        if df.empty or 'Base Currency Equivalent' not in df.columns: return 0.0
        if 'Entry Status' in df.columns:
            df=df[~df['Entry Status'].astype(str).str.lower().eq('voided')].copy()
        if 'Status' in df.columns:
            df=df[df['Status'].astype(str).str.lower().eq('executed')].copy()
        if 'Base Currency' in df.columns:
            df=df[df['Base Currency'].astype(str).str.upper().eq(str(base_currency).upper())].copy()
        if 'Market' in df.columns:
            df=df[df['Market'].astype(str).eq(str(market))].copy()
        if df.empty: return 0.0
        vals=pd.to_numeric(df['Base Currency Equivalent'],errors='coerce').fillna(0.0)
        if 'Side' in df.columns:
            side=df['Side'].astype(str).str.upper()
            vals=np.where(side.eq('SELL'),-vals,vals)
        return max(float(np.sum(vals)),0.0)
    except Exception:
        return 0.0
_selected_market_budget_for_rule=max(float(_market_budget_amount(sel,total_available)),0.0)
_selected_market_actual_for_rule=_early_selected_market_executed_base(sel,currency_code)
_selected_market_target_for_rule=_selected_market_budget_for_rule*deploy_pct
deploy=max(_selected_market_target_for_rule-_selected_market_actual_for_rule,0.0)
cash_deploy,srs_deploy,cpf_deploy,capital_reason=capital_breakdown(zone,deploy,available_cash,available_srs,available_cpf); funding_source='Market Allocation Budget' if deploy>0 else 'No deployment'
...
# CDE: old global title removed; page headers are rendered by the locked router.

# ------------------------- renderers -------------------------

def cde_next_future_trigger(dd_value):
    ladder=[(-8,'INITIAL BUY',.10),(-15,'BUY',.25),(-25,'STRONG BUY',.50),(-35,'CRISIS BUY',.75),(-50,'MAX CRISIS BUY',1.00)]
    try: d=float(dd_value)
    except Exception: return 'N/A','N/A',0.0,'N/A'
    for threshold,label,pct in ladder:
        if d > threshold: return f'{threshold:.0f}%', label, pct, f'{d-threshold:.1f}%'
...
def cde_historical_edge_all_markets(threshold=10):
    ...
    for mk in markets:
        try:
            price_df=m[mk]['df'][['Close']].dropna().copy(); bt=price_df.copy(); bt['rm']=bt.Close.rolling(252,min_periods=1).max(); bt['dd_pct']=((bt.Close-bt.rm)/bt.rm)*100
            ev=crash_events(bt,threshold,safe_float(bt.Close.iloc[-1]),None)
            if ev is None or ev.empty: continue
            all_events.append(ev)
            for _,row in ev.iterrows():
                td=pd.Timestamp(row['Trough Date']); ti=safe_float(row['Trough Index']); pi=safe_float(row['Peak Index'])
                fut=price_df.loc[price_df.index >= td + pd.DateOffset(years=3)]
    ...
    if not all_events: return {'success':'Pending','avg3y':'Pending','recovery':'Pending','worst3y':'Pending','tooltip':'No consolidated crash-event rows were available.'}
    sample=int(sum(len(x) for x in all_events)); valid3=[x for x in forward_3y if pd.notna(x)]
    success=f'{(sum(1 for x in valid3 if x>0)/len(valid3)*100):.0f}%' if valid3 else 'N/A'; avg3y=f'{np.mean(valid3):+.0f}%' if valid3 else 'N/A'; worst3y=f'{np.min(valid3):+.0f}%' if valid3 else 'N/A'; recovery=f'{np.mean(recovery_years):.1f}Y' if recovery_years else 'N/A'
...
def render_executive():
    ...
    _init_capital_settings()
    total_cap=float(_capital_source_breakdown(currency_code).get('total',0.0) or 0.0)
    actual_total_deployed, actual_by_market, deployed_source = _actual_deployed_base_amount(currency_code)
    deployed_amt=max(0.0,min(float(actual_total_deployed),total_cap))
    ...
    # Primary Deployment Action must be executable, not merely the highest opportunity score.
    # Select the highest-scoring triggered market with an assigned budget and an outstanding
    # current-tier deployment. Markets without allocation remain in the watchlist.
    actionable=[]
    for r in rows:
        mk=r['Market']
        active_pct=safe_float(r.get('Deploy %'),deploy_rule(r.get('Drawdown',0)))
        budget=max(float(_market_budget_amount(mk,total_cap)),0.0)
        actual=max(float(actual_by_market.get(mk,0.0)),0.0)
        pending=max(float(pending_by_market.get(mk,0.0)),0.0)
        current_target=budget*active_pct
        outstanding=max(current_target-actual-pending,0.0)
        market_available=max(budget-actual-pending,0.0)
        if active_pct>0 and budget>0 and outstanding>0 and market_available>0 and remaining_amt>0:
            actionable.append((r,outstanding,budget,actual,pending,market_available))
    actionable=sorted(actionable,key=lambda x:(-safe_float(x[0].get('Score'),0),-safe_float(x[0].get('Deploy %'),0),x[0]['Market']))

    if actionable:
        action,action_required,action_market_budget,action_market_actual,action_market_pending,action_market_available=actionable[0]
        action_is_executable=True
    else:
        action=best
        action_is_executable=False
        action_market_budget=max(float(_market_budget_amount(action['Market'],total_cap)),0.0)
        action_market_actual=max(float(actual_by_market.get(action['Market'],0.0)),0.0)
        action_market_pending=max(float(pending_by_market.get(action['Market'],0.0)),0.0)
        action_market_available=max(action_market_budget-action_market_actual-action_market_pending,0.0)
        action_required=0.0

    action_deploy_pct=safe_float(action.get('Deploy %'),0)
    current_trigger_label=str(action.get('Signal','TRIGGER')).upper()
    current_trigger_threshold='-50%' if action_deploy_pct>=1 else '-35%' if action_deploy_pct>=.75 else '-25%' if action_deploy_pct>=.50 else '-15%' if action_deploy_pct>=.25 else '-8%' if action_deploy_pct>=.10 else 'N/A'
    next_trigger,next_tier,next_pct,distance=cde_next_future_trigger(action['Drawdown'])
    next_deploy_amt=min(max(float(action_required),0.0),action_market_available,remaining_amt) if action_is_executable else 0.0
    readiness_required=float(action_required) if action_is_executable else 0.0
    readiness=_funding_readiness_matrix(readiness_required,action_market_budget,action_market_actual,action_market_pending,remaining_amt,action['Market'])
    coverage_ratio=(min(action_market_available,remaining_amt)/readiness_required) if readiness_required>0 else 0
    funding_status=readiness['label']
    ...
    top_funding_status=(f"○ {action['Market']} · {funding_status.upper()}" if action_is_executable else f"○ {best['Market']} · NO ALLOCATION")
    top_funding_status_class=funding_status_class
    dry_powder_tip = tooltip_html('Funding Readiness Detail', _funding_breakdown_pairs(readiness_required,action_market_available,remaining_amt,coverage_ratio,currency_code)+[('Status', funding_status)], readiness['detail'])
    top_funding_amount=f"Portfolio cash: {fmt_sgd_html(remaining_amt)}"
    top_target_warning_html = ''
    # Best Opportunity remains independent from the executable Primary Action.
    best_market_budget=max(float(_market_budget_amount(best['Market'],total_cap)),0.0)
    other_funds_amt=max((srs_balance if 'srs_balance' in globals() else 0)+(cpf_oa_balance if 'cpf_oa_balance' in globals() else 0),0)
    market_rows=[]
    try:
        market_rows=sorted(actual_by_market.items(), key=lambda kv: kv[1], reverse=True) if isinstance(actual_by_market,dict) else []
    except Exception:
        market_rows=[]
    market_preview_items=market_rows[:2]
    more_count=max(len(market_rows)-len(market_preview_items),0)
    if market_preview_items:
        ...
        if more_count>0:
            market_exposure_html+=f'<div class="cde-market-exposure-row"><span>+{more_count} more</span><b></b></div>'
    else:
        market_exposure_html=f'<div class="cde-market-exposure-row"><span>Actual exposure</span><b>{fmt_sgd_html(0)}</b></div>'

    alloc_pct=(deployed_amt/total_cap) if total_cap else 0
    dry_pct=(remaining_amt/total_cap) if total_cap else 0
    active_tier=f"{action['Zone']} / {action_deploy_pct:.0%}"
    confidence_label='High' if action['Score']>=50 else 'Medium' if action['Score']>=25 else 'Low'
    ...
    vol_value='N/A' if vix is None else f'{vix:.1f}'
    risk_needle_deg=max(-90,min(90,(safe_float(global_risk_score,0)/100*180)-90)); risk_gauge_html=f'<div class="cde-risk-mini-meter"><div class="cde-risk-mini-arc"></div><div class="cde-risk-mini-needle" style="--needle:{risk_needle_deg:.0f}deg"></div></div>'
...
''', unsafe_allow_html=True)
    ...
    # A reached trigger with no assigned market allocation is ALLOCATION REQUIRED.
    triggered_items=[]
    next_items=[]
    for r in rows:
        ...
        reached_tier=str(r.get('Signal','TRIGGER')).upper()
        if active_pct>0 and market_budget>0 and outstanding>0:
            triggered_items.append({'Market':market_name,'Status':'ACTION','Tier':reached_tier,'Drawdown':f'{dd_value:.1f}%','Next':trig,'Priority':0,'Score':safe_float(r.get('Score'),0)})
        elif active_pct>0 and market_budget<=0:
            triggered_items.append({'Market':market_name,'Status':'ALLOCATION REQUIRED','Tier':reached_tier,'Drawdown':f'{dd_value:.1f}%','Next':trig,'Priority':1,'Score':safe_float(r.get('Score'),0)})
        else:
            next_items.append({'Market':market_name,'Status':'NEXT','Tier':reached_tier,'Drawdown':f'{dd_value:.1f}%','Next':trig,'DistanceNum':distance_num(dist),'Priority':2,'Score':safe_float(r.get('Score'),0)})
    ...
    watch_rows=''.join([f'''<div class="cde-watch-table-row"><div class="cde-watch-market">{market_label_html(w['Market'])}</div><div class="cde-watch-action"><strong>{hesc(w['Status'])}</strong> · {hesc(w['Tier'])}</div><div class="cde-watch-dd">{hesc(w['Drawdown'])}</div><div class="cde-watch-next">{hesc(w['Next'])}</div></div>''' for w in watch_items])
    watch_html=f'''<div class="cde-watch-table"><div class="cde-watch-table-head"><div>Market</div><div>Action</div><div>Drawdown</div><div>Next</div></div>{watch_rows}</div>'''
    watch_tip=tooltip_html('Deployment Trigger Watchlist',[
        ...
        ('ALLOCATION REQUIRED','Trigger reached but no market allocation is available'),
    ],'Priority: ACTION, then ALLOCATION REQUIRED, then NEXT. Up to three markets are displayed.')
    if action_is_executable:
        condition_text=f"{action['Market']} {current_trigger_label} {current_trigger_threshold} reached"
        next_deployment_display=fmt_sgd_html(next_deploy_amt)
    else:
        condition_text=f"{best['Market']} allocation required"
        next_deployment_display='NO EXECUTABLE ACTION'
    if readiness['code']=='funding_action':
        funding_detail_inline=f"Portfolio funding shortfall<br><span class='cde-readiness-indent'>{fmt_sgd_html(remaining_amt)} cash vs {fmt_sgd_html(readiness_required)} required</span>"
    elif readiness['code']=='ready':
        funding_detail_inline=f"{fmt_sgd_html(action_market_available)} {hesc(action['Market'])} allocation available<br><span class='cde-readiness-indent'>&middot; Coverage {coverage_ratio:.1f}x</span>"
    elif action_is_executable:
        ...
        funding_detail_inline="No allocated market has an outstanding executable deployment"

    st.markdown('''<style>
    .cde-action-status-box.warning{background:#FEF2F2!important;border:1px solid #FECACA!important;box-shadow:none!important;}
    .cde-action-status-box .cde-action-status-title{font-size:13px!important;font-weight:950!important;color:#0F172A!important;margin-bottom:6px!important;line-height:1.15!important;}
    .cde-action-status-box .cde-action-status-line{display:flex!important;align-items:center!important;gap:10px!important;flex-wrap:wrap!important;line-height:1.35!important;}
    .cde-action-status-box .cde-overdeployed-status{display:inline-block!important;margin:0!important;white-space:nowrap!important;font-size:16px!important;font-weight:950!important;color:#059669!important;line-height:1.15!important;}
    .cde-action-status-box .cde-overdeployed-detail{display:inline-block!important;line-height:1.35!important;font-size:10.5px!important;font-weight:800!important;color:#64748B!important;}
    ...
    .cde-command-grid{grid-template-columns:minmax(0,68fr) minmax(360px,32fr)!important;grid-template-areas:"primary watch"!important;}
    .cde-command-grid>.cde-primary-action-card{grid-area:primary!important;}
    .cde-command-grid>.cde-watch-panel-card{grid-area:watch!important;}
    .cde-watch-table{width:100%;margin-top:8px;border-top:1px solid #DDE7F3;}
    .cde-watch-table-head,.cde-watch-table-row{display:grid;grid-template-columns:24% 36% 22% 18%;align-items:center;column-gap:0;}
    .cde-watch-table-head>div{padding:7px 5px;font-size:10px;font-weight:950;color:#475569;background:#F8FAFC;border-bottom:1px solid #DDE7F3;white-space:nowrap;}
    .cde-watch-table-row>div{padding:9px 5px;font-size:10.5px;font-weight:750;color:#0F172A;border-bottom:1px solid #E5EDF6;min-width:0;}
    .cde-watch-panel-card{container-type:inline-size;}
    .cde-watch-market,.cde-watch-dd,.cde-watch-next{white-space:nowrap;}
    .cde-watch-action{font-size:clamp(7.5px,2.55cqw,10.5px)!important;white-space:nowrap!important;line-height:1.15!important;letter-spacing:-.02em!important;overflow-wrap:normal!important;word-break:normal!important;}
    .cde-watch-dd,.cde-watch-next{text-align:center!important;justify-self:stretch!important;font-weight:900!important;}
    .cde-watch-table-head>div:nth-child(3),.cde-watch-table-head>div:nth-child(4){text-align:center!important;}
    @supports not (font-size:1cqw){.cde-watch-action{font-size:clamp(7.5px,.72vw,10.5px)!important;}}
    @media (max-width:900px){.cde-command-grid{grid-template-columns:1fr!important;grid-template-areas:"primary" "watch"!important;}}
    </style>''', unsafe_allow_html=True)
    st.markdown('<div class="cde-dcc-title">Deployment & Capital Command Centre</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="cde-dcc-parent"><div class="cde-command-grid"><div class="cde-primary-action-card"><div class="cde-dcc-section-title">Primary Deployment Action</div><div class="cde-action-row"><div class="cde-action-mini target"><span>Target Market</span><b>{market_label_html(action["Market"])}</b></div><div class="cde-action-mini"><span>Trigger Level</span><b>{hesc(current_trigger_label)} {hesc(current_trigger_threshold)}</b></div><div class="cde-action-mini"><span>Current Drawdown</span><b>{action["Drawdown"]:.1f}%</b></div><div class="cde-action-mini"><span>Distance</span><b>{"Reached" if action_is_executable else hesc(distance)}</b></div><div class="cde-action-mini"><span>Confidence</span><b>{hesc(confidence_label)}</b></div></div><div class="cde-next-action-grid"><div class="cde-next-deployment-hero"><div class="cde-dcc-section-title">Next Deployment</div><div class="cde-next-amount">{next_deployment_display}</div><div class="cde-action-line"><b>Condition:</b> {hesc(condition_text)}</div></div><div class="cde-action-status-box {'warning' if readiness['code'] not in ['ready','no_action'] else ''}"><div class="cde-action-status-title">Funding Readiness — {market_label_html(action["Market"])}</div><div class="cde-action-status-line"><span class="cde-overdeployed-status">{hesc(funding_status)}</span><span class="cde-overdeployed-detail">&middot; {funding_detail_inline}</span></div></div></div></div><div class="cde-watch-panel-card"><div class="cde-watch-title">Deployment Trigger Watchlist {watch_tip}</div>{watch_html}</div></div></div>', unsafe_allow_html=True)
    ...
    with opp_col:
        ...
        with opp_box:
            ...
            .cde-mo-cell{{min-height:34px!important;display:flex!important;align-items:center!important;padding:0 8px!important;background:#FFFFFF!important;border-left:1px solid #DDE7F3!important;border-right:1px solid #DDE7F3!important;border-bottom:1px solid #DDE7F3!important;color:#0F172A!important;font-size:12px!important;font-weight:700!important;line-height:1.05!important;box-sizing:border-box!important;}}
            .cde-mo-center{{justify-content:center!important;text-align:center!important;}}
            .cde-mo-market{{gap:6px!important;font-weight:900!important;}}
            .cde-mo-price{{font-weight:700!important;}}
            .cde-mo-perf{{font-weight:700!important;}}
            .cde-mo-score{{font-weight:900!important;}}
            .cde-mo-drawdown{{color:#F97316!important;font-weight:850!important;}}
            .cde-mo-sig{{justify-content:center!important;}}
            .cde-mo-sig .cde-sig{{display:inline-flex!important;align-items:center!important;justify-content:center!important;min-width:46px!important;height:22px!important;line-height:22px!important;padding:0 8px!important;margin:0!important;font-size:10.5px!important;font-weight:950!important;}}
            ...
            header_cols=st.columns([0.34,1.22,.78,.78,.52,.68,.56,.56,.56,1.00])
            ...
            for i,r in enumerate(rows,1):
                row_cols=st.columns([0.34,1.22,.78,.78,.52,.68,.56,.56,.56,1.00])
                ...
                row_cols[2].markdown(f'<div class="cde-mo-cell cde-mo-center cde-mo-price">{safe_float(r.get("Price"),0):,.0f}</div>', unsafe_allow_html=True)
                row_cols[3].markdown(f'<div class="cde-mo-cell cde-mo-center cde-mo-drawdown">{r["Drawdown"]:.1f}%</div>', unsafe_allow_html=True)
                row_cols[4].markdown(f'<div class="cde-mo-cell cde-mo-center cde-mo-score">{r["Score"]}</div>', unsafe_allow_html=True)
                ...
                row_cols[6].markdown(f'<div class="cde-mo-cell cde-mo-center cde-mo-perf">{fmt_pct_cell(r.get("1Y %"))}</div>', unsafe_allow_html=True)
                row_cols[7].markdown(f'<div class="cde-mo-cell cde-mo-center cde-mo-perf">{fmt_pct_cell(r.get("3Y %"))}</div>', unsafe_allow_html=True)
                row_cols[8].markdown(f'<div class="cde-mo-cell cde-mo-center cde-mo-perf">{fmt_pct_cell(r.get("5Y %"))}</div>', unsafe_allow_html=True)
    ...
    with status_col:
        ...
        st.markdown(f'''<div class="cde-control-card"><div class="cde-control-title">Capital Position</div><div class="cde-control-row"><span>Total Capital</span><b>{fmt_sgd_html(total_cap)}</b><em>100%</em></div><div class="cde-control-row"><span>Actual Deployed</span><b>{fmt_sgd_html(deployed_amt)}</b><em>{alloc_pct:.0%}</em></div><div class="cde-control-row"><span>Cash</span><b>{fmt_sgd_html(remaining_amt)}</b><em>{dry_pct:.0%}</em></div><div class="cde-control-row"><span>{hesc(best['Market'])} Budget</span><b>{fmt_sgd_html(best_market_budget)}</b><em>{_market_allocation_pct(best['Market']):.0%}</em></div></div><div class="cde-exposure-card"><div class="cde-exposure-title">Actual Exposure</div><div>{market_exposure_html}</div></div></div>''', unsafe_allow_html=True)
        if st.button('Portfolio Overview →', key='control_portfolio_trade_log_button', use_container_width=True):
            st.session_state.active_section='📒 Trade Journal'
            st.rerun()
        st.markdown(f'''<div style="margin-top:8px;padding:6px 10px;border:1px solid #D7E3F3;border-radius:12px;background:#F8FBFF;min-height:0;height:auto;line-height:1.15;"><span style="font-size:11px;font-weight:800;color:#0F172A;">✓ System Status</span><span style="font-size:10px;color:#475569;font-weight:700;margin-left:8px;">Systems OK · Data quality High · Model confidence Good</span></div>''', unsafe_allow_html=True)
...
def render_market_deep_dive_summary():
    ...
    cards=[('Inflation',inflation['display'],inflation['sub'],inflation['source_type'],inflation.get('diagnostic','')),('Unemployment',unemployment['display'],unemployment['sub'],unemployment['source_type'],unemployment.get('diagnostic','')), (rate_label,rates['display'],rates['sub'],rates['source_type'],rates.get('diagnostic','')),('PMI',pmi_display,pmi_sub,pmi_src,''),('Yield Curve',_curve(curve_spread),curve_state,'Official API' if curve_spread is not None else 'Awaiting',''),('VIX',f'{vix:.1f}' if vix is not None else 'N/A','Stress input' if vix is not None else 'N/A','Official API' if vix is not None else 'Awaiting','')]
    if index_label in US_MARKETS:
        cards.insert(3, ('Claims',claims['display'],claims['sub'],claims['source_type'],claims.get('diagnostic','')))
    ...
    actual_total_deployed, selected_actual_by_market, selected_deployed_source = _actual_deployed_base_amount(currency_code)
    selected_market_deployed = max(float((selected_actual_by_market or {}).get(index_label, 0.0)), 0.0)
    selected_market_pending = 0.0
    try:
        active_trade_df = _trade_log_with_numeric() if '_trade_log_with_numeric' in globals() else pd.DataFrame()
        if isinstance(active_trade_df, pd.DataFrame) and not active_trade_df.empty:
            if 'Entry Status' in active_trade_df.columns:
                active_trade_df = active_trade_df[~active_trade_df['Entry Status'].astype(str).str.lower().eq('voided')].copy()
            pending_df = active_trade_df[active_trade_df['Status'].astype(str).str.lower().eq('pending')].copy() if 'Status' in active_trade_df.columns else pd.DataFrame()
            if not pending_df.empty and 'Base Currency' in pending_df.columns:
                pending_df = pending_df[pending_df['Base Currency'].astype(str).str.upper().eq(str(currency_code).upper())]
            if not pending_df.empty and 'Market' in pending_df.columns and 'Base Currency Equivalent' in pending_df.columns:
                selected_market_pending = max(float(pending_df[pending_df['Market'].astype(str).eq(str(index_label))]['Base Currency Equivalent'].astype(float).sum()), 0.0)
    except Exception:
        selected_market_pending = 0.0
    selected_market_available_raw = selected_market_budget - selected_market_deployed - selected_market_pending
    selected_market_over_amount = max(-selected_market_available_raw, 0.0)
    selected_market_overdeployed = selected_market_over_amount > 0
...
def render_suggested(expanded=False):
    ...
    def _render_suggested_content():
        ...
        try:
            _init_capital_settings()
            sm_budget=max(float(_market_budget_amount(index_label)),0.0)
            _, sm_actual_by_market, _ = _actual_deployed_base_amount(currency_code)
            sm_deployed=max(float((sm_actual_by_market or {}).get(index_label,0.0)),0.0)
            sm_pending=0.0
            active_trade_df=_trade_log_with_numeric() if '_trade_log_with_numeric' in globals() else pd.DataFrame()
            if isinstance(active_trade_df,pd.DataFrame) and not active_trade_df.empty:
                if 'Entry Status' in active_trade_df.columns:
                ...
                if not pending_df.empty and 'Base Currency' in pending_df.columns:
                    pending_df=pending_df[pending_df['Base Currency'].astype(str).str.upper().eq(str(currency_code).upper())]
                if not pending_df.empty and 'Market' in pending_df.columns and 'Base Currency Equivalent' in pending_df.columns:
                    sm_pending=max(float(pending_df[pending_df['Market'].astype(str).eq(str(index_label))]['Base Currency Equivalent'].astype(float).sum()),0.0)
            sm_available_raw=sm_budget-sm_deployed-sm_pending
            over_amount=max(-sm_available_raw,0.0)
            overdeployed_flag=over_amount>0
        except Exception:
        ...
        holdings_tip=tooltip_html('Top 10 Holdings',[('Scope','Top 10 counters for the selected market, ranked by exposure size'),('% Portfolio','Counter exposure divided by total portfolio capital'),('Price Fetch','Attempts live price fetch for all active holdings, including tickers outside recommended list'),('Fallback','If price is unavailable, % Portfolio uses base-currency cost base and current value/P&L display as N/A')],'Unavailable price/value/P&L displays as N/A, not None. Cost base still counts as Actual Deployed.')
        ladder_tip=tooltip_html('Deployment Ladder',[('Type','Cumulative deployment schedule'),('Trigger Basis','Active structural drawdown'),('Capital Base','Selected investible capital / cash')],'Cumulative deployment percentages by drawdown zone.')
        ...
        if overdeployed_flag:
            ...
            plan_html=f'<div class="cde-mini-plan"><span>Tranche 1</span><b class="orange">{fmt_sgd(deploy*.5)}</b><span>Tranche 2</span><b class="orange">{fmt_sgd(deploy*.25)}</b><span>Tranche 3</span><b class="blue">{fmt_sgd(deploy*.25)}</b><span>Cash capacity</span><b class="green">{fmt_sgd(available_cash)}</b><span>SRS available</span><b class="green">{fmt_sgd(available_srs)}</b><span>CPF-OA available</span><b class="green">{fmt_sgd(available_cpf)}</b><span>Reserved Cash</span><b class="green">{fmt_sgd(cash_deploy)}</b><span>Reserved SRS</span><b class="green">{fmt_sgd(srs_deploy)}</b><span>Reserved CPF-OA</span><b class="green">{fmt_sgd(cpf_deploy)}</b></div>'
        else:
            plan_html=f'<div class="cde-mini-plan"><span>Deploy now</span><b>{fmt_sgd(0)}</b><span>Capital stance</span><b class="green">Preserved</b><span>Next trigger</span><b class="orange">{hesc(next_trigger)}</b></div>'
        ...
        def _current_holdings_table_html():
            try:
                df=_trade_log_with_numeric() if '_trade_log_with_numeric' in globals() else pd.DataFrame()
                if df is None or df.empty: return '<div class="xec-sub">No holding recorded for this market.</div>'
                ...
                if df.empty: return '<div class="xec-sub">No holding recorded for this market.</div>'
                ...
                holding_rows=[]
                total_cost=0.0; total_value=0.0; total_pnl=0.0; any_value=False; all_value=True
                portfolio_base=max(float(total_investible_state if 'total_investible_state' in globals() else st.session_state.get('total_investible_capital_input',0.0) or 0.0),0.0)
                if portfolio_base<=0:
                    portfolio_base=max(float(st.session_state.get('total_investible_capital_input',0.0) or 0.0),0.0)
                        ...
                        try:
                            ...
                            if px_df is not None and not px_df.empty:
                                ...
                                if pd.notna(px) and px>0:
                                    ...
                                    status_cls='cde-status-live'; value_txt=fmt_sgd(mval); pnl_txt=fmt_sgd(pnl); any_value=True; break
                        except Exception: pass
                    exposure_basis=mval if mval is not None else cost
                    pct_port=(exposure_basis/portfolio_base*100.0) if portfolio_base>0 and exposure_basis is not None else np.nan
                    total_cost+=cost
                    if mval is None: all_value=False
                    else: total_value+=mval; total_pnl+=pnl if pnl is not None else 0.0
                    holding_rows.append({'Ticker':ticker_raw,'Qty':qty,'Cost':cost,'MarketValue':mval,'ValueText':value_txt,'PnL':pnl,'PnLText':pnl_txt,'Status':price_status,'StatusClass':status_cls,'ExposureBasis':exposure_basis if exposure_basis is not None else 0.0,'PctPortfolio':pct_port})
                holding_rows=sorted(holding_rows,key=lambda x:safe_float(x.get('ExposureBasis'),0.0),reverse=True)[:10]
                trs=[]; top_cost=0.0; top_value=0.0; top_pnl=0.0; top_exposure=0.0; top_all_value=True; top_any_value=False
                for row in holding_rows:
                    top_cost+=safe_float(row.get('Cost'),0.0); top_exposure+=safe_float(row.get('ExposureBasis'),0.0)
                    if row.get('MarketValue') is None: top_all_value=False
                    else:
                        top_value+=safe_float(row.get('MarketValue'),0.0); top_pnl+=safe_float(row.get('PnL'),0.0); top_any_value=True
                    pct_txt='N/A' if pd.isna(row.get('PctPortfolio')) else f"{safe_float(row.get('PctPortfolio'),0.0):.1f}%"
                    trs.append(f'<tr><td>{hesc(row["Ticker"])}</td><td class="num">{safe_float(row["Qty"]):,.0f}</td><td class="num">{fmt_sgd(row["Cost"])}</td><td class="num">{hesc(row["ValueText"])}</td><td class="num">{hesc(row["PnLText"])}</td><td class="num">{pct_txt}</td><td class="{row["StatusClass"]}">{hesc(row["Status"])}</td></tr>')
                top_value_txt=fmt_sgd(top_value) if top_any_value and top_all_value else ('N/A' if not top_all_value else fmt_sgd(top_value))
                top_pnl_txt=fmt_sgd(top_pnl) if top_any_value and top_all_value else ('N/A' if not top_all_value else fmt_sgd(top_pnl))
                top_pct_txt=f'{(top_exposure/portfolio_base*100):.1f}%' if portfolio_base>0 else 'N/A'
                total_suffix='' if len(grp)<=10 else 'Top 10 Total'
                total_label=total_suffix or 'Total'
                return '<table class="cde-holdings-table"><thead><tr><th>Ticker</th><th>Qty</th><th>Cost</th><th>Market Value</th><th>Gain / Loss</th><th>% Portfolio</th><th>Status</th></tr></thead><tbody>'+''.join(trs)+f'</tbody><tfoot><tr><td>{total_label}</td><td></td><td class="num">{fmt_sgd(top_cost)}</td><td class="num">{top_value_txt}</td><td class="num">{top_pnl_txt}</td><td class="num">{top_pct_txt}</td><td></td></tr></tfoot></table>'
            except Exception:
...
def render_assumptions():
    with st.expander('🧾 Assumptions & Limits — Methodology guardrails', expanded=False):
        assumptions=[
            ...
            'Suggested deploy is based only on the selected investible capital / cash. It is not a buy call, trading instruction, portfolio recommendation, or financial advice.',
...
def render_market(expanded=False):
    with st.expander('📈 Market Valuation & Diagnostics',expanded=expanded):
        st.markdown('## 📈 Market Valuation & Diagnostics')
        st.caption('Supporting valuation, price, drawdown, risk and macro evidence behind the selected-market Deployment Centre.')
        ...
        # 4. Risk score and trigger diagnostics.
        ...
        # 5. Macro-only trend snapshot moved to bottom. No duplicate index price chart here.
...
def fetch_fx_rate_yahoo(from_ccy, to_ccy):
    ...
    for ticker,invert in [(f'{from_ccy}{to_ccy}=X', False), (f'{to_ccy}{from_ccy}=X', True)]:
        try:
            df=hist(ticker, '2026-01-01')
            if df is not None and not df.empty:
                px=safe_float(df.Close.iloc[-1], None)
                if px and px>0:
                    return float((1/px) if invert else px), f'Yahoo Finance {ticker}' + (' inverse' if invert else ''), pd.Timestamp(df.index[-1]).strftime('%Y-%m-%d')
        except Exception:
            pass
    try:
    ...
    try:
        if TRADE_LOG_FILE.exists():
            ...
            for c in TRADE_LOG_COLUMNS:
                if c not in df.columns: df[c]=''
            if 'Funding Source' in df.columns:
                df['Funding Source']=_normalise_funding_source_series(df['Funding Source'])
...
def render_portfolio_trade_journal(view='journal'):
    ...
    st.caption('Portfolio summary, holdings exposure, full trade journal and portfolio-performance attribution.')
    ...
    if st.session_state.get('_portfolio_requested_view') != view:
        st.session_state['_portfolio_requested_view']=view
        st.session_state['portfolio_overview_section_buttons']=default_section
    section=st.session_state.get('portfolio_overview_section_buttons', default_section)
    ...
    if t1.button(('✓ ' if section=='Portfolio Summary' else '')+'Portfolio Summary', key='portfolio_tab_summary', use_container_width=True):
        st.session_state.active_section='📌 Portfolio Summary'; st.session_state['portfolio_overview_section_buttons']='Portfolio Summary'; st.session_state['_portfolio_requested_view']='summary'; st.rerun()
    if t2.button(('✓ ' if section=='Holdings & Exposure' else '')+'Holdings & Exposure', key='portfolio_tab_holdings', use_container_width=True):
        st.session_state.active_section='📦 Holdings & Exposure'; st.session_state['portfolio_overview_section_buttons']='Holdings & Exposure'; st.session_state['_portfolio_requested_view']='holdings'; st.rerun()
    if t3.button(('✓ ' if section=='Trade Journal' else '')+'Trade Journal', key='portfolio_tab_journal', use_container_width=True):
        st.session_state.active_section='📒 Trade Journal'; st.session_state['portfolio_overview_section_buttons']='Trade Journal'; st.session_state['_portfolio_requested_view']='journal'; st.rerun()
    ...
    default_markets=[x for x in st.session_state.get('portfolio_market_filter',[]) if x in market_options]
    default_tickers=[x for x in st.session_state.get('portfolio_ticker_filter',[]) if x in ticker_options]
    f1,f2=st.columns([1,1])
    selected_markets=f1.multiselect('Market filter',market_options,default=default_markets,key='portfolio_market_filter')
    selected_tickers=f2.multiselect('Ticker filter',ticker_options,default=default_tickers,key='portfolio_ticker_filter')
    ...
    total_cap=float(_capital_source_breakdown(base_ccy).get('total',0.0) or 0.0)
    dry=max(total_cap-net_invested,0.0) if total_cap else 0.0
    deploy_pct=(net_invested/total_cap*100) if total_cap else None
    recorded=len(active_filtered)
    pending=int(active_filtered['Status'].astype(str).str.lower().eq('pending').sum()) if not active_filtered.empty and 'Status' in active_filtered.columns else 0
    st.markdown('### Portfolio Position Summary')
    cards=[('Total Capital',fmt_sgd(total_cap)),('Actual Deployed',fmt_sgd(net_invested)),('Cash',fmt_sgd(dry)),('Deployment %',f'{deploy_pct:.0f}%' if deploy_pct is not None else 'N/A'),('Recorded Trades',recorded),('Pending Orders',pending)]
    ...
    def _build_holdings():
        ...
        if 'Funding Source' not in executed.columns:
            executed['Funding Source']='Cash'
        executed['Funding Source']=_normalise_funding_source_series(executed['Funding Source'])
        grp=executed.groupby(['Funding Source','Market','Ticker','Trade Currency','Base Currency'],dropna=False).agg(**{'Net Quantity':('_Signed Quantity','sum'),'Cost Base':('_Signed Base Equivalent','sum'),'Trades':('Ticker','count')}).reset_index()
        vals=[]
        for _,r in grp.iterrows():
            ...
            for cand in _candidate_tickers_for_price(ticker, market):
                try:
                    ...
                    if px_df is not None and not px_df.empty:
                        px=safe_float(px_df.Close.iloc[-1],np.nan)
                        if pd.notna(px) and px>0:
                            cur_px=px; fx,_,_=fetch_fx_rate_yahoo(trade_ccy,base_ccy); cur_val=qty*cur_px*float(fx or 0); pnl=cur_val-cost; price_status='Live' if cand==ticker else f'Live via {cand}'; break
                except Exception:
                    pass
    ...
    def _round(src_df,dec=2):
        ...
        for col in out.columns:
            if pd.api.types.is_numeric_dtype(out[col]): out[col]=out[col].round(dec)
        return out
    holdings=_build_holdings()
    if section=='Portfolio Summary':
        st.markdown('### Market Budget Usage by Market')
        alloc=dict(st.session_state.get('market_allocation_budget_pct',{}) or {}); usage=[]
        deployed_by=executed.groupby('Market')['_Signed Base Equivalent'].sum().to_dict() if not executed.empty and 'Market' in executed.columns else {}
        ...
        if holdings.empty:
            ...
            hnum=holdings.copy()
            for c in ['Current Value','Unrealised P/L','Portfolio Weight %','P/L Contribution %']:
                hnum[c]=pd.to_numeric(hnum[c],errors='coerce')
            attr=hnum.groupby('Market',dropna=False).agg(**{'Cost Base':('Cost Base','sum'),'Current Value':('Current Value','sum'),'Unrealised P/L':('Unrealised P/L','sum'),'Portfolio Weight %':('Portfolio Weight %','sum'),'P/L Contribution %':('P/L Contribution %','sum')}).reset_index()
            st.dataframe(_round(attr,2),use_container_width=True,hide_index=True)
    elif section=='Holdings & Exposure':
        st.markdown('### Holdings & Exposure by Funding Source / Market / Ticker')
        if holdings.empty:
            st.info('No executed active trades available for holdings/exposure summary.')
        else:
            display_holdings=_round(holdings,2).replace({np.nan:'N/A'}).copy()
            for source in ['Cash','SRS','CPF-OA']:
        ...
        st.download_button('Export Trade Journal CSV',df.to_csv(index=False),file_name='cde_trade_journal.csv',mime='text/csv',use_container_width=True)

def render_capital_management_page(view='settings'):
    _init_capital_settings()
    if view=='rules':
        st.markdown('## 🧭 Philosophy, Strategy & Allocation Rules')
        st.caption('Read-only strategy, allocation methodology, deployment ladder and guardrails. Editable market budgets remain only under Capital Settings.')
        st.markdown('### Investment Philosophy')
        st.markdown('- Market crashes create staged deployment opportunities.\n- Capital preservation remains the default stance until trigger levels are reached.\n- Deployment is rules-based, not emotion-based.\n- Each market deploys only from its assigned market allocation budget.\n- Funding readiness must be checked before deployment.')
        st.markdown('### Strategy Framework')
        st.markdown('- Structural drawdown drives the deployment tier.\n- Valuation, macro risk and trend are supporting context.\n- Suggested Deploy is calculated from the selected market allocation budget.\n- Executed trades reduce available market budget.\n- All executed trades are normalised to base currency before portfolio and funding checks.')
    ...
    with left_col:
        with st.expander('Edit Base Capital Setting', expanded=bool(st.session_state.get('_capital_settings_warning',''))):
            ...
            st.markdown("""<style>
            div[data-testid="stButton"] button, div[data-testid="stDownloadButton"] button, div[data-testid="stPopover"] button {min-height:38px;height:38px;}
            </style>""", unsafe_allow_html=True)
            action_col,import_col,export_col=st.columns([1.52,.74,.74])
    ...
    with right_col:
        source_tip=tooltip_html('Capital Source & Readiness Basis',[
            ...
            ('FX treatment','Funding readiness uses base-currency equivalents; transaction currencies and FX rates are captured in Trade Entry')
        ])
...
def run_render_loop():
    ...
    elif active_section in RENDERERS:
