...
def render_executive():
    ...
    cta_left, cta_right = st.columns([0.82,0.18])
    with cta_left:
    ...
    with cta_right:
        if st.button('Deep Dive →', use_container_width=True, key='lp_r1_01_deep_dive_button'):
            ...
            st.rerun()
    st.markdown(f'''<table class="cde-table"><thead><tr><th>Rank</th><th>Market</th><th>Index / ETF</th><th>Drawdown</th><th>Score ⓘ</th><th>Signal</th></tr></thead><tbody>{table_rows}</tbody></table>''', unsafe_allow_html=True)
