...
st.set_page_config(page_title="SG Tactical Wealth Allocator v34", layout="wide", initial_sidebar_state="expanded")
BLUE="#2563EB"; RED="#EF4444"; ORANGE="#F97316"; AMBER="#F59E0B"; GREEN="#16A34A"; SLATE="#64748B"

st.markdown("""
<style>
.block-container{padding-top:1.2rem}.card{border:1px solid #E5E7EB;border-radius:12px;padding:14px 16px;background:white;box-shadow:0 1px 2px rgba(0,0,0,.04)}
.card-title{font-size:12px;color:#64748B;font-weight:600}.card-value{font-size:26px;font-weight:750;color:#111827}.card-sub{font-size:12px;color:#6B7280}
.light-card{background:#F8FAFC;border:1px solid #E5E7EB;border-radius:10px;padding:8px;margin-top:4px}.kv{display:flex;justify-content:space-between;gap:10px;padding:5px 8px;border-bottom:1px solid #E5E7EB}.kvl{color:#6B7280;font-size:13px}.kvv{font-weight:650;font-size:13px;color:#111827;text-align:right}
</style>
""", unsafe_allow_html=True)

INDEX_TICKERS={"S&P 500":"^GSPC","Nasdaq":"^IXIC","DJIA":"^DJI","HSI":"^HSI","STI":"^STI","KLSE":"^KLSE","Gold":"GC=F","Bitcoin":"BTC-USD"}
DISPLAY_NAME={k:k for k in INDEX_TICKERS}
PMI_FRED_MARKETS={"S&P 500","Nasdaq","DJIA"}; PMI_NA_MARKETS={"Gold","Bitcoin"}
PMI_PROXY_MAP={
...
BENCHMARK_TICKERS={"Global Indices":[("STI","^STI"),("Nasdaq","^IXIC"),("S&P 500","^GSPC"),("DJIA","^DJI"),("HSI","^HSI"),("KLSE","^KLSE")],"Commodities & Crypto":[("Crude Oil","CL=F"),("Gold","GC=F"),("Silver","SI=F"),("Bitcoin","BTC-USD")]}
...
def hist(ticker,start="1950-01-01"):
    try:
        df=yf.Ticker(ticker).history(start=start); time.sleep(.03)
...
def live_macro_data():
    def last_close(ticker):
        df=hist(ticker,"2025-01-01"); return None if df.empty else safe_float(df.Close.iloc[-1])
...
def fetch_fred_pmi(series_id="NAPM"):
    try:
        ...
        df=pd.read_csv(url,parse_dates=["DATE"]); df.columns=["Date","PMI"]; df=df.set_index("Date").dropna(); df["PMI"]=pd.to_numeric(df["PMI"],errors="coerce")
        return df.dropna()
    except Exception: return pd.DataFrame()
if "pmi_history" not in st.session_state: st.session_state.pmi_history={}

def card(title,value,sub,accent): return f'<div class="card" style="border-top:4px solid {accent}"><div class="card-title">{title}</div><div class="card-value">{value}</div><div class="card-sub">{sub}</div></div>'
def kv(label,value,colour="#111827"): return f'<div class="kv"><span class="kvl">{label}</span><span class="kvv" style="color:{colour}">{value}</span></div>'
def classify(dd):
...
def current_dd(df,method):
    ...
    peak=safe_float(df.Close.rolling(days,min_periods=1).max().iloc[-1],c); return c,peak,((c-peak)/peak)*100 if peak else 0,label
def deploy_rule(dd):
    if dd<=-35: return .50
    if dd<=-25: return .35
    if dd<=-15: return .20
...
def confidence_score(dd,live_score,trend_below): return max(0,min(100,35+(15 if dd<=-8 else 0)+(10 if trend_below else 0)+(10 if live_score<50 else 0)-(25 if live_score>=70 else 0)))
...
def calc_market_scores(pmi_value,dd_value,trend_weak,vix_value,curve_value,pmi_applicable=True):
    ...
    return (total,"CRASH RISK",vix_s,curve_s,pmi_s,dd_s,trend_s) if total>=70 else (total,"WARNING",vix_s,curve_s,pmi_s,dd_s,trend_s) if total>=50 else (total,"WATCH",vix_s,curve_s,pmi_s,dd_s,trend_s) if total>=30 else (total,"NORMAL",vix_s,curve_s,pmi_s,dd_s,trend_s)
...
def mini_pmi_bar_chart(df,title,subtitle):
    ...
    fig=go.Figure(); fig.add_trace(go.Bar(x=df.index,y=df["PMI"],marker_color=colours,text=[f"{v:.1f}" for v in df["PMI"]],textposition="outside",textfont=dict(size=10,color="#374151"),cliponaxis=False))
...
@st.cache_data(ttl=14400)
def fetch_trend_data(ticker,start="1950-01-01"): return hist(ticker,start)
def build_trend_channel(df,projection_year=2040):
    if df is None or df.empty or len(df)<36: return None
    monthly=df[["Close"]].resample("ME").last().dropna().copy(); monthly["Seq"]=np.arange(1,len(monthly)+1); monthly["LogPrice"]=np.log(monthly["Close"])
    slope,intercept=np.polyfit(monthly["Seq"],monthly["LogPrice"],1); monthly["Trend"]=intercept+slope*monthly["Seq"]; monthly["Residual"]=monthly["LogPrice"]-monthly["Trend"]; sd=monthly["Residual"].std()
    for col,off in [("TrendPrice",0),("Upper1",sd),("Upper2",2*sd),("Lower1",-sd),("Lower2",-2*sd)]: monthly[col]=np.exp(monthly["Trend"]+off)
    monthly["ZHist"]=monthly["Residual"]/sd if sd else 0; z=(monthly["LogPrice"].iloc[-1]-monthly["Trend"].iloc[-1])/sd if sd else 0; pct=(monthly["Residual"]<monthly["Residual"].iloc[-1]).mean()*100; reg_cagr=(np.exp(slope*12)-1)*100
    ...
    last_date=monthly.index[-1]; last_seq=monthly["Seq"].iloc[-1]; proj_dates=pd.date_range(last_date+pd.DateOffset(months=1),f"{projection_year}-12-31",freq="ME"); seq=np.arange(last_seq+1,last_seq+1+len(proj_dates)); tr=intercept+slope*seq
    proj=pd.DataFrame({"TrendPrice":np.exp(tr),"Upper1":np.exp(tr+sd),"Upper2":np.exp(tr+2*sd),"Lower1":np.exp(tr-sd),"Lower2":np.exp(tr-2*sd)},index=proj_dates)
    ext=pd.concat([monthly.nlargest(3,"ZHist")[["Close","ZHist"]],monthly.nsmallest(2,"ZHist")[["Close","ZHist"]]]).sort_index()
    return {"data":monthly,"proj":proj,"sd":sd,"z_score":z,"pct_rank":pct,"reg_cagr":reg_cagr,"actual_cagr":actual_cagr,"extremes":ext}
...
def render_trend_channel(df,market_name):
    ...
    with c1: freq=st.selectbox("Data Frequency",["Monthly","Weekly","Daily"],index=0,key="tc_freq")
    with c2: period=st.selectbox("Regression Period",["Full History","Rolling 15Y","Post-GFC","Post-COVID"],index=0,key="tc_period")
    with c3: proj_year=st.selectbox("Projection Horizon",[2030,2035,2040,2050],index=2,key="tc_proj")
    ticker=TREND_CHANNEL_OVERRIDE.get(market_name,INDEX_TICKERS.get(market_name)); src=fetch_trend_data(ticker) if ticker else df
    if src is None or src.empty: src=df
    ...
    if period=="Rolling 15Y": wdf=wdf.loc[wdf.index>=wdf.index.max()-pd.DateOffset(years=15)]
    elif period=="Post-GFC": wdf=wdf.loc[wdf.index>="2009-01-01"]
    elif period=="Post-COVID": wdf=wdf.loc[wdf.index>="2020-01-01"]
    tc=build_trend_channel(wdf,proj_year)
    if tc is None: st.warning("Insufficient data for trend channel analysis."); return
    tdf=tc["data"]; proj=tc["proj"]; z=tc["z_score"]; status,status_col=_valuation_status(z); dist=((tdf["Close"].iloc[-1]/tdf["TrendPrice"].iloc[-1])-1)*100
    ...
    fig=go.Figure(); fig.add_trace(go.Scatter(x=tdf.index,y=tdf["Close"],name=f"{market_name} Price",line=dict(color=BLUE,width=2))); fig.add_trace(go.Scatter(x=tdf.index,y=tdf["TrendPrice"],name="Trend",line=dict(color="#7C3AED",width=2)))
    bands=[("Upper2","+2 SD",RED),("Upper1","+1 SD",AMBER),("Lower1","-1 SD",GREEN),("Lower2","-2 SD","#059669")]
    for col,lbl,clr in bands: fig.add_trace(go.Scatter(x=tdf.index,y=tdf[col],name=lbl,line=dict(color=clr,dash="dash",width=1.5)))
    if not proj.empty:
        ...
        for col,_,clr in bands: fig.add_trace(go.Scatter(x=proj.index,y=proj[col],line=dict(color=clr,dash="dot",width=1),showlegend=False))
        last=proj.iloc[-1]
        for col,clr in [("Upper2",RED),("Upper1",AMBER),("TrendPrice","#7C3AED"),("Lower1",GREEN),("Lower2","#059669")]: fig.add_annotation(x=proj.index[-1],y=last[col],text=f"<b>{last[col]:,.0f}</b>",showarrow=False,xanchor="left",xshift=5,font=dict(size=11,color=clr))
    for start,end,label in CRISIS_EVENTS:
        s=pd.Timestamp(start); e=pd.Timestamp(end)
        if tdf.index.min()<=e and s<=tdf.index.max():
            x0=max(s,tdf.index.min()); x1=min(e,tdf.index.max()); fig.add_vrect(x0=x0,x1=x1,fillcolor="rgba(239,68,68,0.15)",line=dict(color=RED,width=1),layer="below"); mid=x0+(x1-x0)/2
            fig.add_annotation(x=mid,y=0.95,yref="paper",text=f"<b>{label}</b>",showarrow=False,font=dict(size=11,color="#1F2937"),bgcolor="rgba(255,255,255,0.88)",bordercolor="#D1D5DB",borderwidth=1,borderpad=3)
    fig.add_vline(x=tdf.index[-1].timestamp()*1000,line_dash="dash",line_color=RED,line_width=1.5); fig.add_annotation(x=tdf.index[-1],y=0.95,yref="paper",text=f"<b>Today</b><br>{tdf.index[-1].strftime('%b %d, %Y')}",showarrow=False,font=dict(size=11,color=RED),bgcolor="rgba(255,255,255,0.92)",bordercolor=RED,borderwidth=1,borderpad=3)
    if not proj.empty: fig.add_annotation(x=proj.index[len(proj)//2],y=0.90,yref="paper",text=f"<b>Projection to {proj_year}</b>",showarrow=False,font=dict(size=11,color="#7C3AED"),bgcolor="rgba(255,255,255,0.8)")
    subtitle=f"{freq}, {period} ({tdf.index[0].strftime('%b %Y')} – {tdf.index[-1].strftime('%b %Y')})"; fig.update_layout(height=600,title=dict(text=f"<b>{market_name} 曾氏通道 (Trend Channel Line)</b> — {subtitle}",font=dict(size=15)),yaxis_type="log",yaxis_title="Price (Log Scale)",plot_bgcolor="white",paper_bgcolor="white",margin=dict(l=10,r=90,t=50,b=10),legend=dict(orientation="h",yanchor="bottom",y=-0.12,xanchor="center",x=0.5,font=dict(size=10)))
    st.plotly_chart(fig,use_container_width=True,config={"displayModeBar":False})
    r2c1,r2c2,r2c3,r2c4=st.columns([1,1.2,1.1,.8])
    with r2c1:
        st.markdown("#### 📋 Current Valuation Summary"); cur=tdf["Close"].iloc[-1]; trend=tdf["TrendPrice"].iloc[-1]
        rows=[("Current Price",f"{cur:,.2f}","#111827"),("Trend Value (Log)",f"{trend:,.2f}","#7C3AED"),("Distance from Trend",f"{dist:+.1f}%",ORANGE if dist>0 else GREEN),("Z-Score",f"{z:+.2f}",status_col),("Valuation Zone",status,status_col),("Historical Percentile",f"{tc['pct_rank']:.0f}th","#111827"),("Regression CAGR",f"{tc['reg_cagr']:.2f}%","#111827"),("Actual CAGR",f"{tc['actual_cagr']:.2f}%","#111827"),("Volatility (SD)",f"{tc['sd']:.4f}","#111827"),("Total Months",f"{len(tdf)}","#111827")]
        st.markdown('<div class="light-card">'+''.join([kv(a,b,c) for a,b,c in rows])+'</div>',unsafe_allow_html=True)
    with r2c2:
        st.markdown("#### 📈 Historical Z-Score"); zfig=go.Figure(); zfig.add_trace(go.Scatter(x=tdf.index,y=tdf["ZHist"],mode="lines",line=dict(color=BLUE,width=1.5),fill="tozeroy",fillcolor="rgba(37,99,235,0.12)"))
        for lv,clr in [(2,RED),(1,AMBER),(0,SLATE),(-1,GREEN),(-2,"#059669")]: zfig.add_hline(y=lv,line_dash="dash",line_color=clr,line_width=1)
        zfig.update_layout(height=300,margin=dict(l=10,r=10,t=10,b=10),plot_bgcolor="white",paper_bgcolor="white",showlegend=False,yaxis_title="Z-Score"); st.plotly_chart(zfig,use_container_width=True,config={"displayModeBar":False})
    with r2c3: st.markdown("#### 🎯 Z-Score Guide"); st.dataframe(pd.DataFrame([{"Zone":"🔴 > +2.0","State":"Extreme Overvaluation","Stance":"Very Defensive"},{"Zone":"🟠 +1.0 to +2.0","State":"Expensive","Stance":"Defensive"},{"Zone":"🟡 -1.0 to +1.0","State":"Neutral / Fair","Stance":"Neutral"},{"Zone":"🟢 -2.0 to -1.0","State":"Attractive","Stance":"Accumulation"},{"Zone":"🟢 < -2.0","State":"Extreme Undervaluation","Stance":"Aggressive"}]),use_container_width=True,hide_index=True)
    with r2c4:
        st.markdown("#### 📊 Z-Score"); gfig=go.Figure(go.Indicator(mode="gauge+number",value=z,number={"font":{"size":28}},gauge={"axis":{"range":[-3,3]},"bar":{"color":status_col},"steps":[{"range":[-3,-2],"color":"#D1FAE5"},{"range":[-2,-1],"color":"#A7F3D0"},{"range":[-1,1],"color":"#FEF9C3"},{"range":[1,2],"color":"#FED7AA"},{"range":[2,3],"color":"#FECACA"}],"threshold":{"line":{"color":"#111827","width":3},"value":z}})); gfig.update_layout(height=220,margin=dict(l=20,r=20,t=30,b=10)); st.plotly_chart(gfig,use_container_width=True,config={"displayModeBar":False})
    r3c1,r3c2,r3c3=st.columns([1,1.2,1])
    with r3c1:
        st.markdown("#### 📜 Historical Extremes"); ext=[]
        for dt,row in tc["extremes"].iterrows(): state,_=_valuation_status(row["ZHist"]); ext.append({"Date":dt.strftime("%b %Y"),"Event":_label_extreme(dt),"Z-Score":f"{row['ZHist']:+.2f}","Price":f"{row['Close']:,.0f}","State":state})
        st.dataframe(pd.DataFrame(ext),use_container_width=True,hide_index=True)
    with r3c2:
        st.markdown("#### 🔮 Future Projection"); rows=[]
        for yr in sorted(set([proj_year]+[y for y in [2025,2030,2035,2040] if y<=proj_year])):
            ...
            yv=yd.iloc[-1]; rows.append({"Year":yr,"Trend":f"{yv['TrendPrice']:,.0f}","+1SD":f"{yv['Upper1']:,.0f}","+2SD":f"{yv['Upper2']:,.0f}","-1SD":f"{yv['Lower1']:,.0f}","-2SD":f"{yv['Lower2']:,.0f}"})
        st.dataframe(pd.DataFrame(rows),use_container_width=True,hide_index=True)
    with r3c3:
        st.markdown("#### 🎯 Tactical Implication"); vals=_tactical_implication(z)
        for emoji,lbl,val in [("📊","Valuation",vals[0]),("⚠️","Risk Level",vals[1]),("🧭","Suggested Stance",vals[2]),("📈","Deployment Bias",vals[3])]:
            clr=GREEN if any(x in val for x in ["Accumulation","Increase","Aggressive","Maximum","Low"]) else ORANGE if any(x in val for x in ["Defensive","Reduce","High"]) else BLUE
            st.markdown(f'<div style="padding:6px 10px;margin:4px 0;border-left:3px solid {clr};background:#F8FAFC;border-radius:4px"><span style="font-size:12px;color:#6B7280">{emoji} {lbl}</span><br><span style="font-size:14px;font-weight:650;color:{clr}">{val}</span></div>',unsafe_allow_html=True)
    ...
    for i in range(len(bt)):
        ...
        if dv<=-thr and not in_dd: in_dd=True; start=i
            ...
            if len(ev)==0 or (ti-ev[-1]["Trough Date"]).days>=60:
                look=bt.loc[:ti].iloc[max(0,len(bt.loc[:ti])-252):]; ddv=safe_float(row.dd_pct); price=safe_float(row.Close); peak=safe_float(row.rm); pkdt=look.Close.idxmax(); z,_=classify(ddv); recovery=((current/price)-1)*100 if price else 0
...
with st.sidebar:
    st.markdown("## 📍 Navigation"); active_section=st.radio("Go to section",NAV_OPTIONS,index=0,label_visibility="collapsed"); st.markdown("---"); st.markdown("## ⚙️ Quick Settings")
    sel=st.selectbox("Selected Market",list(m.keys()),index=list(m.keys()).index("STI") if "STI" in m else 0); st.session_state.selected_market_name=DISPLAY_NAME.get(sel,sel)
    st.markdown("### 💰 Capital Pools & Safeguards"); cash_balance=st.number_input("Liquid Cash (S$)",0.0,value=100000.0,step=5000.0); srs_balance=st.number_input("SRS (S$)",0.0,value=35000.0,step=5000.0); cpf_oa_balance=st.number_input("CPF-OA (S$)",0.0,value=180000.0,step=5000.0); emergency_buffer=st.number_input("Emergency Buffer (S$)",0.0,value=20000.0,step=1000.0); preserve_cpf=st.checkbox("Preserve S$20k CPF-OA Floor",value=True); drawdown_method=st.radio("Drawdown Reference",["Rolling 252D Peak","2Y Peak","3Y Peak","5Y Peak","All-Time High Peak"],index=0)
    if st.button("🔄 Refresh Market Data",use_container_width=True): st.cache_data.clear(); st.toast("Market data refreshed.",icon="🔄")
ud=m[sel]["df"]; ticker=m[sel]["ticker"]; index_label=DISPLAY_NAME.get(sel,sel); pmi_proxy_default=PMI_PROXY_MAP.get(sel,{"label":"N/A","region":"N/A","source":"N/A","default":0})
st.title("🇸🇬 Tactical Wealth Allocation & Future Drawdown Simulator v34"); st.caption("Singapore wealth allocation dashboard with market-specific PMI, live risk monitoring, staged deployment, crash analytics and secular valuation channel.")
close,peak,dd,ref=current_dd(ud,drawdown_method); zone,zc=classify(dd); deploy_pct=deploy_rule(dd); available_cash=max(cash_balance-emergency_buffer,0); available_srs=srs_balance; available_cpf=max(cpf_oa_balance-(20000 if preserve_cpf else 0),0); total_available=available_cash+available_srs+available_cpf; deploy=total_available*deploy_pct
...
macro=live_macro_data(); vix=macro.get("vix"); tnx=macro.get("tnx"); irx=macro.get("irx"); curve_spread=(tnx-irx) if (tnx is not None and irx is not None) else None; trend_below=close<m[sel]["ma200"]
pmi_label=pmi_proxy_default["label"]; latest_pmi=float(st.session_state.get("latest_pmi_value",pmi_proxy_default["default"])); pmi_applicable=sel not in PMI_NA_MARKETS
live_score,alert,vix_s,curve_s,pmi_s,dd_s,trend_s=calc_market_scores(latest_pmi,dd,trend_below,vix,curve_spread,pmi_applicable); conf_score=confidence_score(dd,live_score,trend_below); conf_label=confidence_label(conf_score); decision_line=f"Deploy approximately S${deploy:,.0f} using staged tranches." if deploy>0 else "No deployment now. Capital is preserved until a deployment trigger appears."; next_trigger=next_trigger_label(zone)

def render_executive():
    st.markdown("---"); st.markdown("## 🧠 Executive Tactical Allocation Centre"); r1=st.columns(3); r1[0].markdown(card(index_label,f"{close:,.0f}",f"{ticker} · Index Level",BLUE),unsafe_allow_html=True); r1[1].markdown(card("Current Drawdown",f"{dd:.1f}%",ref,RED),unsafe_allow_html=True); r1[2].markdown(card("Current Market Action",zone,"Drawdown-based rule",ORANGE),unsafe_allow_html=True); r2=st.columns(3); r2[0].markdown(card("Suggested Deploy",f"S${deploy:,.0f}","Calculation output",AMBER),unsafe_allow_html=True); r2[1].markdown(card("Risk Regime",alert,f"Live Risk Score: {live_score:.0f} / 100",RED if alert=="CRASH RISK" else ORANGE if alert=="WARNING" else AMBER if alert=="WATCH" else GREEN),unsafe_allow_html=True); r2[2].markdown(card("Signal Confidence",conf_label,f"Approx. {conf_score:.0f} / 100",BLUE),unsafe_allow_html=True); st.markdown(f"**Formula used:** Current drawdown = (current close − selected peak reference) ÷ selected peak reference. **Selected reference:** {ref} at approximately **{peak:,.0f}**.  \n**Decision note:** {decision_line}")
...
def get_pmi_df(chosen,latest_in):
    ...
    if market in PMI_NA_MARKETS: return pd.DataFrame()
    if market in PMI_FRED_MARKETS:
        fred=fetch_fred_pmi("NAPM")
        if not fred.empty: return fred.last("12ME")
...
def render_market(expanded=False):
    with st.expander("🌦️ MARKET CONDITIONS & LIVE RISK MONITOR",expanded=expanded):
        st.markdown("## 🌦️ Market Conditions & Live Risk Monitor"); current_proxy=st.session_state.get("pmi_proxy_label",pmi_proxy_default["label"]); current_proxy=current_proxy if current_proxy in PMI_PROXY_OPTIONS else (pmi_proxy_default["label"] if pmi_proxy_default["label"] in PMI_PROXY_OPTIONS else "N/A"); actual=LATEST_PMI_ACTUALS.get(current_proxy,LATEST_PMI_ACTUALS["N/A"]); st.markdown(f"### LIVE MARKET RISK ALERT: {alert}"); st.caption(f"Rules-based stress indicator, not a crash prediction. PMI proxy used: {current_proxy}.")
        ...
        with p2: pmi_region=st.text_input("PMI Region",value=PMI_PROXY_MAP.get(sel,{}).get("region","N/A"))
        with p3: pmi_source_in=st.text_input("PMI Source",value=st.session_state.get("latest_pmi_source",actual["source"]))
        with p4: latest_in=st.number_input("Latest PMI",0.0,70.0,float(st.session_state.get("latest_pmi_value",actual["value"])),step=.1)
        ...
        with p6:
            ...
            if st.button("🔄 Update PMI",use_container_width=True):
                ...
                if market in PMI_FRED_MARKETS:
                    fred=fetch_fred_pmi("NAPM")
                    if not fred.empty:
                        latest_val=float(fred["PMI"].iloc[-1]); latest_month=fred.index[-1].strftime("%b %Y"); st.session_state.latest_pmi_value=latest_val; st.session_state.latest_pmi_month=latest_month; st.session_state.latest_pmi_source="FRED (ISM Manufacturing PMI)"; st.session_state.pmi_proxy_label="US ISM Manufacturing PMI"; st.session_state.pmi_history["US ISM Manufacturing PMI"]={d.strftime("%Y-%m"):float(v) for d,v in fred.last("12ME")["PMI"].items()}; st.toast(f"✅ ISM PMI fetched from FRED: {latest_val:.1f} for {latest_month}",icon="🔄")
                ...
                st.rerun()
        with p7: st.markdown("<br>",unsafe_allow_html=True); st.toggle("Manual",value=sel not in PMI_FRED_MARKETS and sel not in PMI_NA_MARKETS)
        pmi_app=sel not in PMI_NA_MARKETS; latest_display=0.0 if not pmi_app else latest_in; local_score,local_alert,lvix,lcurve,lpmi,ldd,ltrend=calc_market_scores(latest_display,dd,trend_below,vix,curve_spread,pmi_app)
        st.caption("PMI is monthly, not intraday. US markets fetch actual FRED data on button click; non-US markets save manual inputs; Gold/Bitcoin are N/A."); cols=st.columns(5); cols[0].metric("VIX Live","N/A" if vix is None else f"{vix:.1f}"); cols[1].metric("Yield Curve","N/A" if curve_spread is None else f"10Y-13W {curve_spread:.2f}%"); cols[2].metric(chosen,"N/A" if not pmi_app else f"{latest_in:.1f}"); cols[3].metric(f"{index_label} Drawdown",f"{dd:.1f}%"); cols[4].metric("Live Risk Score",f"{local_score:.0f}/100")
        sig,trigger,engine=st.columns([1,1,1.15])
        with sig: st.markdown("#### 📊 Signal Confidence Details"); st.markdown('<div class="light-card">'+kv("Drawdown Signal","Active" if dd<=-8 else "Inactive",ORANGE if dd<=-8 else SLATE)+kv("Macro Stress",local_alert,RED if local_alert=="CRASH RISK" else ORANGE if local_alert=="WARNING" else AMBER if local_alert=="WATCH" else GREEN)+kv("Technical Trend","Weak" if trend_below else "Stable",BLUE)+'</div>',unsafe_allow_html=True)
        with trigger: st.markdown("#### 📡 Live Trigger Monitor"); trig=pd.DataFrame([{"Trigger":"VIX > 25","Status":"Yes" if vix is not None and vix>25 else "No"},{"Trigger":"Yield curve inverted","Status":"Yes" if curve_spread is not None and curve_spread<0 else "No"},{"Trigger":f"{chosen} < 50","Status":"N/A" if not pmi_app else "Yes" if latest_in<50 else "No"},{"Trigger":"Drawdown < -10%","Status":"Yes" if dd<-10 else "No"},{"Trigger":"Below 200D MA","Status":"Yes" if trend_below else "No"}]); st.dataframe(trig,use_container_width=True,hide_index=True)
        with engine: st.markdown("#### 🧮 Live Risk Score Engine"); st.markdown('<div class="light-card">'+kv("VIX Score",f"{lvix:.0f} / 30",AMBER)+kv("Yield Curve Score",f"{lcurve:.0f} / 20",BLUE)+kv(f"{chosen} Score",f"{lpmi:.0f} / 20",GREEN)+kv("Drawdown Score",f"{ldd:.0f} / 25",ORANGE)+kv("Trend Score",f"{ltrend:.0f} / 15",RED)+kv("Total",f"{local_score:.0f} / 100 → {local_alert}",RED if local_alert=="CRASH RISK" else ORANGE if local_alert=="WARNING" else GREEN)+'</div>',unsafe_allow_html=True)
        with st.expander("📈 12M Trend Snapshot",expanded=False):
            ...
            pmi_df=get_pmi_df(chosen,latest_in); idx12=ud.loc[ud.index>=ud.index.max()-pd.DateOffset(months=12)][["Close"]].rename(columns={"Close":"Index"}); ch1,ch2=st.columns(2)
...
def render_crash(expanded=False):
    with st.expander("🏆 Crash & Recovery Analytics",expanded=expanded):
        st.markdown("## 📊 Executive Crash & Cycle Summary"); st.caption("Historical drawdown severity bands: 10–20%, 20–30%, and >30%."); p,q=st.columns([1,1])
        ...
        bt=ud.loc[pd.Timestamp(start):].copy(); bt["rm"]=bt.Close.rolling(252,min_periods=1).max(); bt["dd_pct"]=((bt.Close-bt.rm)/bt.rm)*100; cur=safe_float(bt.Close.iloc[-1]); event_df=crash_events(bt,thr,cur)
        ...
        for c in ["Peak Date","Trough Date"]: display[c]=pd.to_datetime(display[c]).dt.strftime("%Y-%m-%d")
        for c in ["Peak Index","Trough Index","Drawdown %","Recovery Return %"]: display[c]=display[c].round(1)
        st.dataframe(display,use_container_width=True,hide_index=True); st.download_button("⬇️ Export Crash Events CSV",display.to_csv(index=False),file_name="crash_events.csv",mime="text/csv")
        with st.expander("🧪 Master Crash Deployment Simulator",expanded=False):
            s1,s2=st.columns(2)
            with s1: inv=st.number_input("Investment per event (S$)",min_value=1000.0,value=10000.0,step=1000.0,key="master_invest")
            ...
            end_index=safe_float(end_slice.Close.iloc[-1]); sim=event_df[pd.to_datetime(event_df["Trough Date"])<=pd.Timestamp(end_date)].copy()
            if sim.empty: st.info("No events before selected end date."); return
            sim["Investment Amount"]=inv; sim["End Index"]=end_index; sim["Ending Value"]=inv*(sim["End Index"]/sim["Trough Index"]); sim["Gain / Loss"]=sim["Ending Value"]-sim["Investment Amount"]; sim["Return %"]=(sim["Ending Value"]/sim["Investment Amount"]-1)*100; total=sim["Investment Amount"].sum(); ending=sim["Ending Value"].sum(); tr=(ending/total-1)*100 if total else 0; c1,c2,c3,c4=st.columns(4); c1.metric("Deployments",len(sim)); c2.metric("Capital Deployed",f"S${total:,.0f}"); c3.metric("Ending Value",f"S${ending:,.0f}"); c4.metric("Total Return",f"{tr:.1f}%"); st.dataframe(sim[["Trough Date","Historical Label","Severity","Zone","Trough Index","End Index","Investment Amount","Ending Value","Gain / Loss","Return %"]],use_container_width=True,hide_index=True)

def render_audit(expanded=False):
    with st.expander("📡 AUDIT TRAIL & EXPORT",expanded=expanded):
        left,right=st.columns([1,1])
        with left: st.markdown("#### 📡 Data Source & Freshness"); st.markdown('<div class="light-card">'+kv("Market Data","Yahoo Finance",BLUE)+kv("PMI Proxy",st.session_state.get("pmi_proxy_label",pmi_label),GREEN)+kv("PMI Value",f"{st.session_state.get('latest_pmi_value',latest_pmi):.1f} · {st.session_state.get('latest_pmi_month','')}",GREEN)+kv("PMI Source",st.session_state.get("latest_pmi_source",pmi_proxy_default["source"]),GREEN)+kv("Last Refreshed",datetime.now().strftime('%d %b %Y %H:%M SGT'),SLATE)+'</div>',unsafe_allow_html=True)
        with right: st.markdown("#### 🧾 Methodology Notes"); st.markdown("- Live Risk Score is rules-based and not a crash prediction.\n- PMI is monthly, not intraday live data.\n- US PMI is fetched from FRED only when Update PMI is clicked.\n- Non-US PMI uses manual input with pre-filled 12M defaults.\n- Gold / Bitcoin PMI is not applicable.")
        snap=pd.DataFrame([{"Timestamp":datetime.now().strftime('%Y-%m-%d %H:%M:%S SGT'),"Selected Index":index_label,"Ticker":ticker,"Drawdown Reference":ref,"Current Drawdown %":round(dd,2),"Action Zone":zone,"Suggested Deploy S$":round(deploy,2),"Funding Source":funding_source,"PMI Proxy":st.session_state.get("pmi_proxy_label",pmi_label),"PMI Value":st.session_state.get("latest_pmi_value",latest_pmi),"Live Risk Score":round(live_score,1),"Risk Regime":alert,"Signal Confidence":conf_label}]); st.markdown("#### 📤 Tactical Snapshot Export"); st.dataframe(snap,use_container_width=True,hide_index=True); st.download_button("⬇️ Export Tactical Snapshot CSV",snap.to_csv(index=False),file_name="tactical_snapshot.csv",mime="text/csv")
