    ...
    for i in range(len(bt)):
        ...
        if dv<=-thr and not in_dd:
            ...
            if len(ev)==0 or (ti-ev[-1]["date"]).days>=60:
                ...
                ev.append({"date":ti,"price":price,"dd":ddv,"zone":z,"peak":peak,"peak_dt":pkdt,"ret":((current/price)-1)*100 if price else 0})
    return ev
...
def render_crash(expanded=False):
    ...
    with st.expander("🏆 CRASH & RECOVERY ANALYTICS",expanded=expanded):
        ...
        try:
            p,q=st.columns(2)
            with p:
                start=st.date_input("Historical analysis start date",value=ud.index.min().date(),min_value=ud.index.min().date(),max_value=ud.index.max().date())
            with q:
                thr=st.slider("Minimum drawdown threshold (%)",5,50,10,5)
            ...
            ev=crash_events(bt,thr,cur)
            if not ev:
                st.info("No drawdown events found with the selected parameters.")
            else:
                k1,k2,k3,k4,k5=st.columns(5)
                rets=[t['ret'] for t in ev]
                with k1: st.metric("Crash Events",len(ev))
                with k2: st.metric("Success Rate",f"{sum(1 for r in rets if r>0)/len(rets)*100:.0f}%")
                with k3: st.metric("Avg Recovery",f"{np.mean(rets):.1f}%")
                with k4: st.metric("Best Recovery",f"{max(rets):.1f}%")
