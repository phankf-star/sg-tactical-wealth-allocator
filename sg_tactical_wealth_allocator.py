...
def build_drawdown_events(bt, bt_threshold, bt_amount, current_level):
    # Previous version formula retained: rolling 252D high = rm; dd_pct = (Close - rm) / rm * 100
    events = []
    in_dd = False
    ep_s = None
...
sel_idx = st.selectbox(
    "Select Market Index",
    list(market.keys()),
    index=list(market.keys()).index("Hang Seng Index (HK Cyclical/Beta)") if "Hang Seng Index (HK Cyclical/Beta)" in market else 0,
)
...
<b>Formula used:</b> Current drawdown = (current close − selected peak reference) ÷ selected peak reference. Current reference is <b>{drawdown_label}</b> at approximately <b>{drawdown_peak:,.0f}</b>.<br>
<b>Current tactical interpretation:</b> {sel_idx} is in <b>{zone}</b> territory with a drawdown of <b>{current_dd:.1f}%</b>.
</div>
    ...
    try:
        ...
        if not troughs:
            ...
            if filtered_df.empty:
                ...
                crash_speed = "Fast crash" if days_to_trough <= 90 else ("Medium-speed bear market" if days_to_trough <= 365 else "Slow grinding bear market")
