...
.severity-card {{border-radius:12px;padding:16px 18px;min-height:74px;border:1px solid transparent;font-weight:600;}}
.blue-card {{background:#DBEAFE;border-color:#BFDBFE;color:#075985;}}
.yellow-card {{background:#FEF9C3;border-color:#FEF08A;color:#92400E;}}
.red-card {{background:#FEE2E2;border-color:#FECACA;color:#B91C1C;}}
</style>
...
def severity_label(dd):
    if dd<=-30: return ">30% crash"
    if dd<=-20: return "20–30% correction"
    if dd<=-10: return "10–20% correction"
    return "Below threshold"

def historical_label(date):
...
def detect_drawdown_events(bt,threshold,current_price):
    events=[]; in_dd=False; start_i=None
    for i in range(len(bt)):
        dv=bt.dd_pct.iloc[i]
        if dv<=-threshold and not in_dd:
            in_dd=True; start_i=i
        elif dv>-5 and in_dd:
            in_dd=False
            segment=bt.iloc[start_i:i]
            if segment.empty: continue
            trough_date=segment.dd_pct.idxmin(); trough=bt.loc[trough_date]
            if events and (trough_date-events[-1]["Trough Date"]).days<60: continue
            peak_window=bt.loc[:trough_date].iloc[max(0,len(bt.loc[:trough_date])-252):]
            peak_date=peak_window.Close.idxmax(); peak_value=safe_float(peak_window.Close.max())
            dd=safe_float(trough.dd_pct); price=safe_float(trough.Close); zone,_=classify(dd)
            events.append({"Peak Date":peak_date,"Peak Index":peak_value,"Trough Date":trough_date,"Trough Index":price,"Drawdown %":dd,"Recovery Return %":((current_price/price)-1)*100 if price else 0,"Zone":zone,"Severity":severity_label(dd),"Historical Label":historical_label(trough_date)})
    return events
...
with st.sidebar:
    ...
    st.markdown("- 🧠 Executive Centre")
    st.markdown("- 💰 Suggested Deploy")
    ...
    default_idx=list(m.keys()).index("Hang Seng Index (HK Cyclical/Beta)") if "Hang Seng Index (HK Cyclical/Beta)" in m else 0
    sel=st.selectbox("Selected Market",list(m.keys()),index=default_idx)
...
funding_source="Cash First" if cash_deploy>0 else "No deployment"; decision_line="Deploy a small initial tranche only. Preserve SRS and CPF-OA for deeper drawdown zones." if deploy>0 else "No deployment now. Capital is preserved until a deployment trigger appears."
...
st.markdown(f"<div class='section-card'><b>Formula used:</b> Current drawdown = (current close − selected peak reference) ÷ selected peak reference.<br><b>Selected reference:</b> {ref} at approximately <b>{peak:,.0f}</b>.<br><br><b>Decision note:</b> {decision_line}</div>",unsafe_allow_html=True)

# =========================================================
# 2. Suggested Deploy Basis & Capital Source
# =========================================================
with st.expander("💰 Suggested Deploy Basis & Capital Source",expanded=(deploy>0)):
    s1,s2,s3,s4=st.columns([1,1,1,1.1])
    with s1:
        st.markdown(f"<div class='preview-panel'><h3 style='margin-top:0'>📌 Suggested Deploy Basis</h3><p>Suggested Deploy = Available Deployable Capital × Deployment Rule</p><h2 style='margin-top:0;color:{AMBER_TEXT}'>S${deploy:,.0f} = S${total_available:,.0f} × {deploy_pct:.0%}</h2></div>",unsafe_allow_html=True)
    with s2:
        st.markdown("<div class='preview-panel'><h3 style='margin-top:0'>🏦 Capital Source Breakdown</h3>"+preview_row("Funding Source",funding_source,GREEN)+preview_row("Cash Deployment",f"S${cash_deploy:,.0f}",GREEN)+preview_row("SRS Deployment",f"S${srs_deploy:,.0f}",SLATE)+preview_row("CPF-OA Deployment",f"S${cpf_deploy:,.0f}",SLATE)+f"<div class='note-amber'>Reason: {capital_reason}</div></div>",unsafe_allow_html=True)
    with s3:
        if deploy>0:
    ...
    with s4:
        st.markdown("<div class='preview-panel'><h3 style='margin-top:0'>🧭 Deployment Ladder & 🛑 Safeguards</h3>"+preview_row("HOLD / small drawdown","0% deploy",SLATE)+preview_row("INITIAL BUY","10% deploy · Cash only",AMBER)+preview_row("BUY","20–35% deploy · Cash then SRS",ORANGE)+preview_row("STRONG BUY","50% deploy · Cash + SRS + CPF-OA",RED)+preview_row("Hard-stop flags",f"{len(flags)} active",RED if flags else GREEN)+"</div>",unsafe_allow_html=True)

# =========================================================
# 3. Market Conditions & Live Risk Monitor
# =========================================================
with st.expander("🌦️ MARKET CONDITIONS & LIVE RISK MONITOR",expanded=False):
    st.markdown(f"<div class='alert-card {klass}'><h2 style='margin:0'>LIVE MARKET RISK ALERT: {alert}</h2><div class='small-note'>PMI controls, signal confidence, live triggers and risk-score engine.</div></div>",unsafe_allow_html=True)
...
# =========================================================
# 5. Crash & Recovery Analytics — updated with severity guide inside summary
# =========================================================
with st.expander("🏆 CRASH & RECOVERY ANALYTICS",expanded=False):
    ...
    with sev1: st.markdown("<div class='severity-card blue-card'>📉 10–20% corrections represent normal correction-zone events in this dashboard.</div>",unsafe_allow_html=True)
    with sev2: st.markdown("<div class='severity-card yellow-card'>⚠️ 20–30% corrections represent deeper bear-market drawdowns.</div>",unsafe_allow_html=True)
    with sev3: st.markdown("<div class='severity-card red-card'>🚨 >30% crashes represent severe crash-regime drawdowns.</div>",unsafe_allow_html=True)
    st.markdown("---")
    c1,c2=st.columns([1,1])
    with c1: start=st.date_input("Historical analysis start date",value=ud.index.min().date(),min_value=ud.index.min().date(),max_value=ud.index.max().date())
    with c2: threshold=st.slider("Minimum drawdown threshold (%)",5,50,10,5)
    ...
    bt["rolling_peak"]=bt.Close.rolling(252,min_periods=1).max()
    bt["dd_pct"]=((bt.Close-bt.rolling_peak)/bt.rolling_peak)*100
    current_price=safe_float(bt.Close.iloc[-1]) if not bt.empty else close
    events=detect_drawdown_events(bt,threshold,current_price) if not bt.empty else []
    event_df=pd.DataFrame(events)
    k1,k2,k3,k4,k5=st.columns(5)
    if event_df.empty:
        with k1: st.metric("Crash Events",0)
        with k2: st.metric("Success Rate","N/A")
        with k3: st.metric("Avg Recovery","N/A")
        with k4: st.metric("Best Recovery","N/A")
        with k5: st.metric("Current Drawdown",f"{dd:.1f}%")
        st.info("No historical events matched the selected threshold and start date.")
    else:
        success_rate=(event_df["Recovery Return %"]>0).mean()*100
        with k1: st.metric("Crash Events",len(event_df))
        with k2: st.metric("Success Rate",f"{success_rate:.0f}%")
        with k3: st.metric("Avg Recovery",f"{event_df['Recovery Return %'].mean():.1f}%")
        with k4: st.metric("Best Recovery",f"{event_df['Recovery Return %'].max():.1f}%")
        with k5: st.metric("Current Drawdown",f"{bt.dd_pct.iloc[-1]:.1f}%")
        st.markdown("---")
        st.markdown("## 🔍 Interactive Event Explorer")
        f1,f2,f3=st.columns(3)
        with f1: sev_filter=st.multiselect("Severity filter",["10–20% correction","20–30% correction",">30% crash"],default=["10–20% correction","20–30% correction",">30% crash"])
        with f2: zone_options=sorted(event_df["Zone"].dropna().unique().tolist())
        with f2: zone_filter=st.multiselect("Buy zone filter",zone_options,default=zone_options)
        with f3: label_options=["All"]+sorted(event_df["Historical Label"].dropna().unique().tolist())
        with f3: label_filter=st.selectbox("Historical label group",label_options,index=0)
        filt=event_df[event_df["Severity"].isin(sev_filter)&event_df["Zone"].isin(zone_filter)].copy()
        if label_filter!="All": filt=filt[filt["Historical Label"]==label_filter]
        out=filt.copy()
        for col in ["Peak Date","Trough Date"]:
            out[col]=pd.to_datetime(out[col]).dt.strftime("%Y-%m-%d")
        num_cols=["Peak Index","Trough Index","Drawdown %","Recovery Return %"]
        for col in num_cols:
            out[col]=out[col].round(1)
        st.dataframe(out[["Peak Date","Peak Index","Trough Date","Trough Index","Drawdown %","Recovery Return %","Severity","Zone","Historical Label"]],use_container_width=True,hide_index=True)
        st.download_button("⬇️ Export Crash Analytics CSV",out.to_csv(index=False),file_name="crash_recovery_analytics.csv",mime="text/csv")
