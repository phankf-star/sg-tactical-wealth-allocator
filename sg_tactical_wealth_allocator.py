...
MARKET_UPLOAD_ALIASES = {
    'S&P 500':'US','Nasdaq':'US','DJIA':'US','STI':'SG','HSI':'HK','A-Share':'CN','KLSE':'MY','Nikkei 225':'JP','Gold':'GLOBAL','Bitcoin':'GLOBAL'
}
...
MACRO_SOURCE_REGISTRY = {
    ...
    'HSI': {'Inflation':'HKMA economic statistics API','Jobs':'HKMA economic statistics API'},
    'STI': {'Inflation':'SingStat / data.gov.sg official adapter pending mapping','Jobs':'SingStat / MOM official adapter pending mapping'},
    'A-Share': {'Inflation':'NBS official adapter pending validation','Jobs':'NBS official adapter pending validation','PMI':'NBS official adapter pending validation'},
}
...
def us_macro_dashboard_data():
    ...
    out={'inflation_yoy':None,'inflation_date':'N/A','jobs_rate':None,'jobs_date':'N/A','claims_k':None,'claims_date':'N/A'}
    try:
        if len(cpi) >= 13:
            latest=float(cpi['Value'].iloc[-1]); prior=float(cpi['Value'].iloc[-13])
            if prior:
                out['inflation_yoy']=((latest/prior)-1)*100; out['inflation_date']=pd.Timestamp(cpi.index[-1]).strftime('%b %Y')
    except Exception: pass
    try:
        if not unrate.empty:
            out['jobs_rate']=float(unrate['Value'].iloc[-1]); out['jobs_date']=pd.Timestamp(unrate.index[-1]).strftime('%b %Y')
    except Exception: pass
    try:
        if not claims.empty:
            out['claims_k']=float(claims['Value'].iloc[-1])/1000.0; out['claims_date']=pd.Timestamp(claims.index[-1]).strftime('%d %b %Y')
    except Exception: pass
    return out
    ...
    try:
        ...
        if records:
            ...
            if 'end_of_month' in df.columns:
                ...
                df=df.sort_values('end_of_month')
            if 'composite_cpi' in df.columns:
                s=pd.to_numeric(df['composite_cpi'], errors='coerce').dropna()
                if len(s)>=13:
                    out['inflation_value']=((float(s.iloc[-1])/float(s.iloc[-13]))-1)*100
                    out['inflation_date']=df.loc[s.index[-1],'end_of_month'].strftime('%b %Y') if 'end_of_month' in df.columns and pd.notna(df.loc[s.index[-1],'end_of_month']) else 'Latest'
            if 'unemploy_rate' in df.columns:
                s=pd.to_numeric(df['unemploy_rate'], errors='coerce').dropna()
                if len(s)>0:
                    out['jobs_rate']=float(s.iloc[-1])
                    out['jobs_date']=df.loc[s.index[-1],'end_of_month'].strftime('%b %Y') if 'end_of_month' in df.columns and pd.notna(df.loc[s.index[-1],'end_of_month']) else 'Latest'
    except Exception:
        pass
    return out
...
def _normalise_macro_upload(df):
    ...
    return df[required+['source_type','notes']]
...
def resolve_macro_value(market, indicator):
    # 1. Official API adapters where stable.
    if market in US_MARKETS:
        data=us_macro_dashboard_data()
        if indicator=='Inflation' and data.get('inflation_yoy') is not None:
            return {'value':data['inflation_yoy'],'display':f"{data['inflation_yoy']:.1f}%",'sub':f"Official API · FRED CPI YoY · {data['inflation_date']}",'source_type':'Official API'}
        if indicator=='Jobs' and data.get('jobs_rate') is not None:
            return {'value':data['jobs_rate'],'display':f"{data['jobs_rate']:.1f}%",'sub':f"Official API · FRED UNRATE · {data['jobs_date']}",'source_type':'Official API'}
        if indicator=='Claims' and data.get('claims_k') is not None:
            return {'value':data['claims_k'],'display':f"{data['claims_k']:.0f}k",'sub':f"Official API · FRED ICSA · {data['claims_date']}",'source_type':'Official API'}
    if market=='HSI' and indicator in ['Inflation','Jobs']:
        ...
        if indicator=='Inflation' and data.get('inflation_value') is not None:
            return {'value':data['inflation_value'],'display':f"{data['inflation_value']:.1f}%",'sub':f"Official API · HKMA CPI YoY · {data['inflation_date']}",'source_type':'Official API'}
        if indicator=='Jobs' and data.get('jobs_rate') is not None:
            return {'value':data['jobs_rate'],'display':f"{data['jobs_rate']:.1f}%",'sub':f"Official API · HKMA unemployment · {data['jobs_date']}",'source_type':'Official API'}
    # 2. Owner-upload CSV/XLSX fallback.
...
def render_macro_data_manager_sidebar():
    with st.expander('📥 Macro Data Manager', expanded=False):
        st.caption('Source priority: Official API → official downloadable table → owner-upload CSV/XLSX → awaiting input.')
...
def render_executive():
    ...
    macro_tip=tooltip_html('Macro Data Source Priority',[('Priority','Official API → official download → owner-upload → awaiting input'),('Current market',index_label)],'Macro cards are diagnostic context only and do not change Suggested Deploy unless the scoring model is explicitly revised.')
    ...
    def _source_class(src):
        return 'source-official' if src=='Official API' else 'source-upload' if src=='Owner-uploaded' else 'source-awaiting'
    macro_html=''.join([f'''<div class="xec-card xec-micro-card {'unavailable' if str(v).startswith('Awaiting') or v=='N/A' else ''}"><div class="xec-micro-name">{hesc(n)} {tooltip_html(n,[('Source',src),('Status',s)],'Macro data source priority: Official API, official download, owner-upload, awaiting input.')}</div><div class="xec-micro-value {'muted' if str(v).startswith('Awaiting') or v=='N/A' else ''}">{hesc(v)}</div><div class="xec-micro-sub">{hesc(s)} <span class="source-pill {_source_class(src)}">{hesc(src)}</span></div></div>''' for n,v,s,src in cards])
