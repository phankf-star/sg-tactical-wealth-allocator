...
.cde-kpi-grid .cde-card{height:154px;min-height:154px;box-sizing:border-box;display:flex;flex-direction:column;justify-content:space-between;overflow:hidden}.cde-kpi-grid .cde-card-title{min-height:26px}.cde-kpi-grid .cde-main-value{line-height:1.05}.cde-nav-group-caption{font-size:11px;color:#64748b;font-weight:750;margin:2px 0 8px}.cde-card+.cde-card{margin-top:12px}.cde-command-capital-grid{display:grid;grid-template-columns:30% 70%;gap:12px;margin:8px 0 14px}.cde-dcc-table{border:1px solid #0f172a;border-radius:0;background:#fff;overflow:hidden}.cde-dcc-row-title{font-size:14px;font-weight:950;color:#0f172a;padding:4px 8px;border-bottom:1px solid #0f172a;background:#fff}.cde-dcc-action-row{display:grid;grid-template-columns:repeat(5,1fr);border-bottom:1px solid #0f172a;min-height:72px}.cde-dcc-cell{padding:8px;border-right:1px solid #0f172a;min-height:72px}.cde-dcc-cell:last-child{border-right:0}.cde-dcc-cell span{display:block;font-size:11px;color:#0f172a;font-weight:850;margin-bottom:8px}.cde-dcc-cell b{font-size:17px;color:#0f172a}.cde-dcc-cell.target{background:#e2f0d9}.cde-dcc-bottom{display:grid;grid-template-columns:40% 60%;min-height:118px}.cde-dcc-bottom-card{padding:8px;border-right:1px solid #0f172a}.cde-dcc-bottom-card:last-child{border-right:0}.cde-dcc-bottom-card h4{font-size:14px;font-weight:950;margin:0 0 4px;color:#0f172a}.cde-dcc-line{font-size:13px;color:#0f172a;margin:4px 0}.cde-market-exposure{margin-top:4px;font-size:12px;color:#0f172a;font-weight:800}.cde-market-exposure-row{display:flex;justify-content:space-between;gap:8px;padding:2px 0}.cde-dcc-button-wrap{text-align:right;margin-top:10px}.cde-small-action-button{display:inline-block;border:1px solid #d3dfec;border-radius:9px;padding:7px 12px;font-size:12px;font-weight:850;color:#0f172a;background:#fff}.cde-dcc-real-btn div[data-testid="stButton"]{text-align:right}
...
def render_executive():
    ...
    st.markdown(f'''<div class="cde-section-title">Deployment & Capital Command Centre</div><div class="cde-section-sub">Next trigger monitoring, next deployment and capital exposure.</div><section class="cde-command-capital-grid"><div class="cde-watch-card"><div class="cde-watch-title">Next Trigger Watchlist</div><div class="cde-watch-sub">Top nearby trigger levels</div>{watch_html}</div><div class="cde-dcc-table"><div class="cde-dcc-row-title">Action Target</div><div class="cde-dcc-action-row"><div class="cde-dcc-cell target"><span>Target Market</span><b>{market_label_html(best['Market'])}</b></div><div class="cde-dcc-cell"><span>Trigger Level</span><b>{hesc(next_trigger)}</b></div><div class="cde-dcc-cell"><span>Current Drawdown</span><b>{best['Drawdown']:.1f}%</b></div><div class="cde-dcc-cell"><span>Distance</span><b>{hesc(distance)}</b></div><div class="cde-dcc-cell"><span>Confidence</span><b>{hesc(confidence_label)}</b></div></div><div class="cde-dcc-bottom"><div class="cde-dcc-bottom-card"><h4>Next Deployment</h4><div class="cde-dcc-line">Next Deployment: <b>{fmt_sgd_html(next_deploy_amt)}</b></div><div class="cde-dcc-line">Condition: <b>{hesc(condition_text)}</b></div><div class="cde-dcc-line">Available Dry Powder: <b>{fmt_sgd_html(remaining_amt)}</b></div><div class="cde-dcc-line">Coverage: <b>{hesc(coverage_text)}</b></div></div><div class="cde-dcc-bottom-card"><h4>Capital Exposure</h4><div class="cde-dcc-line">Total Capital: <b>{fmt_sgd_html(total_cap)}</b></div><div class="cde-dcc-line">Current Allocated / Deployed by Market</div>{market_exposure_html}<div class="cde-dcc-button-wrap"><span class="cde-small-action-button">Portfolio & Trade Log →</span></div></div></div></div></section>''', unsafe_allow_html=True)
    st.markdown('<div class="cde-dcc-real-btn">', unsafe_allow_html=True)
