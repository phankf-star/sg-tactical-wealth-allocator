...
def render_executive():
    ...
    flag_class_map={'HSI':'hk','KLSE':'my','STI':'sg','A-Share':'cn','Nasdaq':'us','S&P 500':'us','DJIA':'us','Nikkei 225':'jp'}
    flag_text_map={'HSI':'HK','KLSE':'MY','STI':'SG','A-Share':'CN','Nasdaq':'US','S&P 500':'US','DJIA':'US','Nikkei 225':'JP'}
    def market_display_html(market_name):
        cls=flag_class_map.get(market_name,'us'); txt=flag_text_map.get(market_name,'')
        return f'<span class="flag-chip flag-{cls}">{txt}</span>{hesc(market_name)}'
    def market_display_text(market_name):
        return f"{flag_text_map.get(market_name,'')} {market_name}".strip()
    ...
    deployed_amt=total_cap*max_deploy_pct
    remaining_amt=max(total_cap-deployed_amt,0)
    next_trigger,next_tier,next_pct,distance=cde_next_future_trigger(best['Drawdown'])
    active_tier=f"{best['Zone']} / {best['Deploy %']:.0%}"
    edge=cde_historical_edge_all_markets()
    table_rows=''.join([f'''<tr><td>{i}</td><td>{market_display_html(r['Market'])}</td><td>{hesc(r['Index / ETF'])}</td><td class="cde-orange">{r['Drawdown']:.1f}%</td><td>{r['Score']}</td><td><span class="cde-sig {'buy' if r['Signal']=='BUY' else 'watch' if r['Signal']=='WATCH' else 'hold'}">{hesc(r['Signal'])}</span></td></tr>''' for i,r in enumerate(rows,1)])
    ...
    donut_deg=int(max_deploy_pct*360)
