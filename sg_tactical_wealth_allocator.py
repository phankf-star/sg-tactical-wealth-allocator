...
def render_suggested(expanded=False):
    with st.expander("💰 Suggested Deploy Basis & Capital Source", expanded=expanded):
        ...
        with s4:
            ...
            st.markdown('<div class="light-card">' + kv("HOLD / small drawdown", "0% deploy", SLATE) + kv("INITIAL BUY", "10% deploy · Cash only", AMBER) + kv("BUY", "20–35% deploy · Cash then SRS", ORANGE) + kv("STRONG BUY", "50% deploy · Cash + SRS + CPF-OA", RED) + kv("Next Trigger", next_trigger, ORANGE) + '</div>', unsafe_allow_html=True)
