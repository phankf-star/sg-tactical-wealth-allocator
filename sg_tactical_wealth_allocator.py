...
.cde-kpi-grid .cde-card{height:154px;min-height:154px;box-sizing:border-box;display:flex;flex-direction:column;justify-content:space-between;overflow:hidden}.cde-kpi-grid .cde-card-title{min-height:26px}.cde-kpi-grid .cde-main-value{line-height:1.05}.cde-env-compact{margin-top:4px!important}.cde-nav-group-caption{font-size:11px;color:#64748b;font-weight:750;margin:2px 0 8px}.cde-card+.cde-card{margin-top:12px}.cde-command-capital-grid{display:grid;grid-template-columns:30% 70%;gap:12px;margin:8px 0 14px}.cde-dcc-level-grid{display:grid;grid-template-columns:1.15fr 1fr 1.15fr;gap:14px}.cde-dcc-card{background:#fff;border:1px solid #d3dfec;border-radius:14px;padding:13px;min-height:212px}.cde-dcc-card-title{font-size:15px;font-weight:950;color:#0f172a;margin-bottom:10px}.cde-action-target-grid{display:grid;grid-template-columns:1fr 1fr 1fr 1fr 1fr;gap:8px}.cde-action-target-mini{background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:9px;min-height:74px}.cde-action-target-mini.target{background:#ecfdf5;border-color:#bbf7d0}.cde-action-target-mini span{display:block;font-size:9.8px;color:#64748b;font-weight:950;letter-spacing:.02em}.cde-action-target-mini b{display:block;font-size:16px;color:#0f172a;margin-top:5px}.cde-nextdeploy-main{font-size:25px;font-weight:950;color:#0f172a;line-height:1.1}.cde-nextdeploy-line{font-size:12px;color:#475569;font-weight:850;margin-top:6px}.cde-exposure-metrics{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:10px}.cde-exposure-metrics div{background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:9px}.cde-exposure-metrics b{display:block;font-size:17px;color:#059669}.cde-exposure-metrics span{font-size:10.5px;color:#64748b;font-weight:850}.cde-market-exposure{border-top:1px solid #e5e7eb;padding-top:7px;margin-top:7px;font-size:11.2px;color:#475569;font-weight:850}.cde-market-exposure-row{display:flex;justify-content:space-between;gap:8px;border-bottom:1px solid #eef2f7;padding:3px 0}.cde-dcc-button-wrap{margin-top:10px}.cde-small-action-button{display:inline-block;border:1px solid #d3dfec;border-radius:999px;padding:6px 12px;font-size:11.5px;font-weight:900;color:#2563eb;background:#fff}
...
def render_executive():
    ...
    st.markdown(f'''<div class="cde-section-title">Deployment & Capital Command Centre</div><div class="cde-section-sub">Next trigger monitoring, next deployment and capital exposure.</div><section class="cde-command-capital-grid"><div class="cde-watch-card"><div class="cde-watch-title">Next Trigger Watchlist</div><div class="cde-watch-sub">Top nearby trigger levels</div>{watch_html}</div><div class="cde-action-card"><div class="cde-dcc-level-grid"><div class="cde-dcc-card"><div class="cde-dcc-card-title">1. Action Target</div><div class="cde-action-target-grid"><div class="cde-action-target-mini target"><span>Target Market</span><b>{market_label_html(best['Market'])}</b></div><div class="cde-action-target-mini"><span>Trigger Level</span><b>{hesc(next_trigger)}</b></div><div class="cde-action-target-mini"><span>Current Drawdown</span><b>{best['Drawdown']:.1f}%</b></div><div class="cde-action-target-mini"><span>Distance</span><b>{hesc(distance)}</b></div><div class="cde-action-target-mini"><span>Confidence</span><b>{hesc(confidence_label)}</b></div></div></div><div class="cde-dcc-card"><div class="cde-dcc-card-title">2. Next Deployment</div><div class="cde-nextdeploy-main">{fmt_sgd_html(next_deploy_amt)}</div><div class="cde-nextdeploy-line"><b>Condition:</b> {hesc(condition_text)}</div><div class="cde-nextdeploy-line"><b>Available Dry Powder:</b> {fmt_sgd_html(remaining_amt)}</div><div class="cde-nextdeploy-line"><b>Coverage:</b> {hesc(coverage_text)}</div></div><div class="cde-dcc-card"><div class="cde-dcc-card-title">3. Capital Exposure</div><div class="cde-exposure-metrics"><div><b>{fmt_sgd_html(total_cap)}</b><span>Total Capital</span></div><div><b>{fmt_sgd_html(deployed_amt)}</b><span>Allocated</span></div><div><b>{fmt_sgd_html(remaining_amt)}</b><span>Dry Powder</span></div></div><div class="cde-card-sub"><b>Current Allocated / Deployed by Market</b></div>{market_exposure_html}<div class="cde-dcc-button-wrap"><span class="cde-small-action-button">Portfolio & Trade Log →</span></div></div></div></div></section>''', unsafe_allow_html=True)
