...
def render_executive():
    ...
    st.markdown(f'<div class="cde-dcc-parent"><div class="cde-command-grid"><div class="cde-primary-action-card"><div class="cde-dcc-section-title">Primary Deployment Action</div><div class="cde-action-row"><div class="cde-action-mini target"><span>Target Market</span><b>{market_label_html(best["Market"])}</b></div><div class="cde-action-mini"><span>Trigger Level</span><b>{hesc(next_trigger)}</b></div><div class="cde-action-mini"><span>Current Drawdown</span><b>{best["Drawdown"]:.1f}%</b></div><div class="cde-action-mini"><span>Distance</span><b>{hesc(distance)}</b></div><div class="cde-action-mini"><span>Confidence</span><b>{hesc(confidence_label)}</b></div></div><div class="cde-next-action-grid"><div class="cde-next-deployment-hero"><div class="cde-dcc-section-title">Next Deployment</div><div class="cde-next-amount">{fmt_sgd_html(next_deploy_amt)}</div><div class="cde-action-line"><b>Condition:</b> {hesc(condition_text)}</div></div><div class="cde-action-status-box" style="background:{'#FEF2F2' if best_market_overdeployed else '#F8FAFC'}!important;border:1px solid {'#FECACA' if best_market_overdeployed else '#E2E8F0'}!important;box-shadow:none!important;"><div style="font-size:13px;font-weight:950;color:#0F172A!important;margin-bottom:6px;">Funding Readiness — {market_label_html(best["Market"])}</div><div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;line-height:1.35;"><div style="display:inline-block;margin:0;white-space:nowrap;font-size:16px;font-weight:950;color:{'#DC2626' if best_market_overdeployed else '#059669'}!important;">{hesc(funding_status)}</div><div style="display:inline-block;line-height:1.35;font-size:10.5px;font-weight:850;color:{'#7F1D1D' if best_market_overdeployed else '#64748B'}!important;">&middot; {funding_detail_inline}</div></div></div></div></div><div class="cde-watch-panel-card"><div class="cde-watch-title">Next Trigger Watchlist</div><div class="cde-watch-sub">Top nearby trigger levels</div>{watch_html}</div></div></div>', unsafe_allow_html=True)
