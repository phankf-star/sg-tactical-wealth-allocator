...
def render_executive():
    ...
    st.markdown(f'''<div class="cde-command-compact"><div class="cde-command-title">Deployment Command Centre</div><div class="cde-command-sub">Compact next deployment trigger panel. Figures are system-calculated.</div><div class="cde-command-top"><div class="cde-command-mini"><div class="cde-command-label">Target Market</div><div class="cde-command-value">{market_label_html(best['Market'])}</div></div><div class="cde-command-mini"><div class="cde-command-label">Trigger Level</div><div class="cde-command-value">{hesc(next_trigger)}</div></div><div class="cde-command-mini"><div class="cde-command-label">Current Drawdown</div><div class="cde-command-value">{best['Drawdown']:.1f}%</div><div class="cde-command-note">{hesc(active_tier)}</div></div><div class="cde-command-mini"><div class="cde-command-label">Distance</div><div class="cde-command-value">{hesc(distance)}</div></div><div class="cde-command-mini"><div class="cde-command-label">Confidence</div><div class="cde-command-value">{hesc(confidence_label)}</div></div></div><div class="cde-command-bottom"><div class="cde-command-panel"><div class="cde-card-title">Allocation</div><div class="cde-card-sub">Cash available {fmt_sgd_html(remaining_amt)} · Deployed {fmt_sgd_html(deployed_amt)} · Other funds {fmt_sgd_html(0)}</div></div><div class="cde-command-next"><div class="cde-command-label">Next Deployment Amount</div><div class="cde-command-value">{fmt_sgd_html(next_deploy_amt)}</div><div class="cde-command-note">{next_increment:.0%} incremental</div></div><div class="cde-command-panel"><div class="cde-card-title">Condition</div><div class="cde-card-sub">{hesc(condition_text)}</div></div></div></div><section class="cde-grid cde-bottom-grid"><div class="cde-card cde-takeaway"><div class="cde-icon">⊙</div><div><div class="cde-card-title">Key Takeaway</div><div class="cde-card-sub">Opportunities are ranked across all supported equity markets. Single-market execution and all original details remain inside Market Deep Dive.</div></div></div><div class="cde-card"><div class="cde-card-title">Historical Edge — All Markets {edge_tip}</div>{edge_html}</div><div class="cde-card cde-takeaway"><div class="cde-icon">✓</div><div><div class="cde-card-title">System Status</div><div class="cde-card-sub">All systems operational<br>Data quality: High<br>Model confidence: Good</div></div></div></section>''', unsafe_allow_html=True)
