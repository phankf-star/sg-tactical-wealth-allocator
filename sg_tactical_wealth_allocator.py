...
with st.sidebar:
    ...
    with st.expander('Capital Management', expanded=True):
        st.markdown('<div class="cde-nav-group-caption">Capital settings, methodology and safeguard rules</div>', unsafe_allow_html=True)
...
def render_suggested(expanded=False):
    ...
    def _render_suggested_content():
        ...
        st.markdown(f'''<div class="xec-visible-section-title">{hesc(suggested_title)}</div><section class="xec-action-grid"><div class="xec-card xec-action-card"><h4>📌 Calculation Basis</h4><div class="xec-sub">Suggested Deploy = Allocated Market Capital × Deployment Rule − Actual Deployed</div><div class="xec-formula">{current_currency_html()}{deploy:,.0f} =<br>{current_currency_html()}{_selected_market_budget_for_rule:,.0f} × {deploy_pct:.0%} − {current_currency_html()}{_selected_market_actual_for_rule:,.0f}</div><div class="xec-note">Source: selected price data, structural drawdown formula, selected market allocation budget and executed market exposure. This is the calculation basis, not a buy call.</div></div><div class="xec-card xec-action-card"><h4>🏦 Capital Source Breakdown</h4><div>{capital_rows}</div></div></section><section class="xec-action-grid"><div class="{action_class}"><h4>{action_title}</h4>{action_html}</div><div class="xec-card xec-action-card"><h4>🧭 Deployment Ladder — Cumulative Investible Capital {ladder_tip}</h4><div>{kv('HOLD / NO DEPLOYMENT','0% cumulative deploy',SLATE)+kv('INITIAL BUY · -8%','10% cumulative · cash first',BLUE)+kv('BUY · -15%',buy_label,AMBER)+kv('STRONG BUY · -25%',strong_label,ORANGE)+kv('CRISIS BUY · -35%','75% cumulative deploy',RED)+kv('MAX CRISIS BUY · -50%','100% cumulative investible capital',PURPLE)+kv('Next Trigger',next_trigger,ORANGE)}</div></div></section>''', unsafe_allow_html=True)
