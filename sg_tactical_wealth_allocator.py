...
st.caption('A dynamic live-updating Singapore wealth allocation platform with regime classification, opportunity scoring, and crash-buying validation.')
...
st.sidebar.markdown('---')
st.sidebar.markdown('## \u2699\ufe0f Risk Safeguards')
emergency_buffer = st.sidebar.number_input('Emergency Buffer ($)', min_value=0.0, value=20000.0, step=1000.0)
preserve_cpf = st.sidebar.checkbox('Preserve S$20k CPF-OA Floor', value=True, help='Protects the extra 1% bonus yield.')
...
ETF_UNIVERSE = {
    ...
    'Hang Seng Index (HK Cyclical/Beta)': {'label': '\U0001f1ed\U0001f1f0 Hong Kong', 'etfs': [('Tracker Fund (TraHK)','2800.HK'),('iShares HSI ETF','3115.HK'),('iShares HS TECH ETF','3067.HK')]},
    'Nasdaq 100 (Tech Growth)': {'label': '\U0001f1fa\U0001f1f8 Nasdaq', 'etfs': [('Invesco QQQ Trust','QQQ'),('Invesco NASDAQ 100','QQQM')]},
    'S&P 500 (US Market Core)': {'label': '\U0001f1fa\U0001f1f8 S&P 500', 'etfs': [('SPDR S&P 500 (SPY)','SPY'),('Vanguard S&P 500 (VOO)','VOO'),('iShares Core S&P 500','IVV')]},
...
BENCHMARK_TICKERS = {
    'Global Indices': [('Straits Times Index','^STI'),('Nasdaq','^IXIC'),('S&P 500','^GSPC'),('DJIA','^DJI'),('Nikkei 225','^N225'),('SSE A Share','000002.SS'),('TWSE','^TWII')],
...
def safe_float(val, fb=1000.0):
    try:
        v=float(val)
        if math.isnan(v) or math.isinf(v): return fb
        return v
    except: return fb
    ...
    for nm,tk in INDEX_TICKERS.items():
        try:
            ...
            if not df.empty:
                ...
                m[nm]={'live_close':cs,'ma_200':ma,'ath_peak':atp,'drawdown':dd,'underlying_df':df}
...
with st.spinner('Fetching macro indicators...'): live_macro=fetch_macro()
if not mdb: st.error('\U0001f6a8 No data. Try Force Refresh.'); st.stop()

st.markdown('### \U0001f52e Market Conditions & Scenario Modeler')
st.info('Live data loaded. Adjust sliders for scenario analysis.')
avail=list(mdb.keys())
sel_idx=st.selectbox('Select Target Index',avail)
if sel_idx not in mdb: st.error('Data unavailable.'); st.stop()
...
with r1c1:
    ...
    use_hist=st.checkbox('Use Historical Date Price',value=False)
    if use_hist:
        td_=st.date_input('Pick Date',value=mxd,min_value=mnd,max_value=mxd)
        ci=ud.index.get_indexer([pd.Timestamp(td_)],method='nearest')[0]
        pp=float(ud.iloc[ci]['Close']); st.caption(f'Price: **{pp:,.2f}**')
        dup=ud.loc[:pd.Timestamp(td_)]; rw=dup.iloc[max(0,len(dup)-252):]
        tp=float(rw['Close'].max()); pd_=rw['Close'].idxmax()
    else: pp=None
with r1c2:
    if use_hist:
...
if not use_hist:
    rw=ud.iloc[max(0,len(ud)-252):]; tp=float(rw['Close'].max()); pd_=rw['Close'].idxmax()
try: pds=pd_.strftime('%Y-%m-%d')
except: pds='N/A'
...
with r2c1:
    pmi_in=st.slider('US ISM PMI',40.0,60.0,51.5,help='Below 50 = contraction.')
...
with r2c2:
    ys_in=st.slider('Yield Spread (10Y\u22123M)',-1.50,2.50,yd_,help='Below 0 = inverted.')
    if ls_ is not None:
        y10=live_macro.get('yield_10y',0); y3=live_macro.get('yield_3m',0)
        st.caption(f'\U0001f4e1 **{ls_:.2f}%** (10Y:{y10:.2f}% \u2212 3M:{y3:.2f}%)')
with r2c3:
    vix_in=st.slider('CBOE VIX',10.0,80.0,vd_,help='Above 30 = fear.')
...
with cc2:
    try:
        ...
        if th_ is not None and ih_ is not None:
            ...
            st.caption('3M as proxy. 2Y unavailable via free API.')
...
if rsk>=3 or edd<=-35: regime='CRISIS'; regime_color='#B71C1C'; regime_emoji='\U0001f534'; regime_deploy=0.75; regime_label='Maximum contrarian deployment'
elif rsk>=2 or edd<=-20: regime='RISK-OFF'; regime_color='#E65100'; regime_emoji='\U0001f7e0'; regime_deploy=0.50; regime_label='Elevated contrarian opportunity'
elif rsk>=1 or edd<=-10: regime='NEUTRAL'; regime_color='#F9A825'; regime_emoji='\U0001f7e1'; regime_deploy=0.15; regime_label='Selective deployment'
else: regime='RISK-ON'; regime_color='#2E7D32'; regime_emoji='\U0001f7e2'; regime_deploy=0.25; regime_label='Standard deployment pace'
...
if edd<=-20 and mt: trend_score = 10  # contrarian buy signal
...
avail_capital = max(0.0, cash_balance - emergency_buffer) + srs_balance + (max(0.0, cpf_oa_balance - 20000.0) if preserve_cpf else cpf_oa_balance)
deploy_amount = avail_capital * regime_deploy
...
elif rsk>=1: rbg='#FFE0B2';rbd='#E65100';ri='\u26a0\ufe0f';rtc='#E65100';rl='Elevated: '+str(rsk)+' factor(s)'
...
bh+='<div style="background:rgba(255,255,255,0.2);border-radius:8px;padding:8px 16px"><div style="font-size:11px;color:'+ztc+';opacity:.7">Deploy</div><div style="font-size:16px;font-weight:700;color:'+ztc+'">'+str(int(regime_deploy*100))+'%</div></div>'
bh+='</div></div>'
...
with ec3:
    ...
    h+='<div style="font-size:12px;color:#777">'+str(int(regime_deploy*100))+'% of S$'+f'{avail_capital:,.0f}'+'</div></div>'
    st.markdown(h,unsafe_allow_html=True)
with ec4:
    ...
    h+='<div style="font-size:12px;color:#999">Dry powder for future opportunities</div></div>'
    st.markdown(h,unsafe_allow_html=True)
...
with st.expander('\U0001f4ca Opportunity Score Breakdown'):
    ...
    sc_tbl += f'| Trend (vs 200MA) | {"Below" if mt else "Above"} | **{trend_score}/20** |' + chr(10)
...
elif azn=='STRONG SELL': st.warning('\u26a0\ufe0f Bubble. Pausing.')
...
st.markdown('---')
st.subheader(f'Total Deploy: :green[S${co+so+cpfo:,.2f}]')
...
st.caption('What if you invested $10,000 at every major drawdown trough in ' + sel_idx + '?')
try:
    bt=ud.copy(); bt['rm']=bt['Close'].rolling(252,min_periods=1).max(); bt['dd']=((bt['Close']-bt['rm'])/bt['rm'])*100
    lc=float(bt['Close'].iloc[-1]); troughs=[]; in_dd=False; ep_start=None
    for i in range(len(bt)):
        dv=bt['dd'].iloc[i]
        if dv<=-10 and not in_dd: in_dd=True; ep_start=i
        elif dv>-5 and in_dd:
            in_dd=False; episode=bt.iloc[ep_start:i]; ti=episode['dd'].idxmin(); tr=bt.loc[ti]
            if len(troughs)==0 or (ti-troughs[-1]['date']).days>=60:
                d_=float(tr['dd']); p_=float(tr['Close'])
                z_='STRONG BUY' if d_<=-35 else ('BUY' if d_<=-20 else 'INITIAL BUY')
                zc_='#D32F2F' if d_<=-35 else ('#E65100' if d_<=-20 else '#F9A825')
                troughs.append({'date':ti,'price':p_,'dd':d_,'zone':z_,'zc':zc_,'cv':10000*(lc/p_),'ret':((lc/p_)-1)*100})
    if in_dd and ep_start:
        episode=bt.iloc[ep_start:]; ti=episode['dd'].idxmin(); tr=bt.loc[ti]
        if len(troughs)==0 or (ti-troughs[-1]['date']).days>=60:
            d_=float(tr['dd']); p_=float(tr['Close'])
            z_='STRONG BUY' if d_<=-35 else ('BUY' if d_<=-20 else 'INITIAL BUY')
            zc_='#D32F2F' if d_<=-35 else ('#E65100' if d_<=-20 else '#F9A825')
    ...
    if troughs:
        ...
        with sc1: st.metric('Total Invested',f'${ti_:,.0f}',help=f'{len(troughs)} events')
        with sc2: st.metric('Value Today',f'${tc_:,.0f}',f'{tr_:+.1f}%')
        with sc3: st.metric('Avg Return',f'{sum(t["ret"] for t in troughs)/len(troughs):,.1f}%')
        # Table
        th_='<table style="width:100%;border-collapse:collapse;font-size:13px;margin:16px 0"><thead><tr style="background:#F0F2F6;border-bottom:2px solid #DDD">'
        th_+='<th style="padding:8px">#</th><th style="padding:8px">Date</th><th style="text-align:center;padding:8px">Level</th><th style="text-align:center;padding:8px">DD</th><th style="text-align:center;padding:8px">Zone</th><th style="text-align:center;padding:8px">Invested</th><th style="text-align:center;padding:8px">Value</th><th style="text-align:center;padding:8px">Return</th></tr></thead><tbody>'
        ...
        # SUCCESS RATE TABLE
        st.markdown('#### \U0001f4ca Historical Crash Buying Success Probability')
        thresholds = [5, 10, 15, 20, 30, 40]
        sr_tbl = '| Correction Depth | Events | Profitable | Success Rate |' + chr(10) + '|:---|:---:|:---:|:---:|' + chr(10)
        for thr in thresholds:
            events_at = [t for t in troughs if t['dd'] <= -thr]
            wins = [t for t in events_at if t['ret'] > 0]
            rate = (len(wins)/len(events_at)*100) if events_at else 0
            emoji = '\U0001f7e2' if rate >= 80 else ('\U0001f7e1' if rate >= 60 else '\U0001f534')
            sr_tbl += f'| \u2265 {thr}% drawdown | {len(events_at)} | {len(wins)} | {emoji} **{rate:.0f}%** |' + chr(10)
        st.markdown(sr_tbl)
        st.success('\U0001f4a1 **Every major crash in ' + sel_idx + ' was a buying opportunity.** $' + f'{ti_:,.0f}' + ' invested became $' + f'{tc_:,.0f}' + ' \u2014 ' + f'{tr_:.1f}%' + ' total return.')
    else: st.info('No drawdown events found.')
except Exception as e: st.warning(f'Backtest unavailable: {e}')

st.markdown('---')
st.caption('\u26a0\ufe0f Disclaimer: Educational purposes only. Not financial advice. Past performance does not guarantee future results. Consult a licensed advisor.')
