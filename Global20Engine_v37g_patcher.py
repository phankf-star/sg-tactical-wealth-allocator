...
DEFAULT_INPUT = "sg_tactical_wealth_allocator.py"
DEFAULT_OUTPUT = "sg_tactical_wealth_allocator_v37g.py"
...
def patch_source(src: str) -> str:
    ...
    src = src.replace(
        "generator_version','value':'Global20Engine v37f web macro pack generator'",
        "generator_version','value':'Global20Engine v37g web macro pack generator'"
    )

    src = re.sub(
        r"@st\.cache_data\(ttl=21600\)\s+@st\.cache_data\(ttl=21600\)",
        "@st.cache_data(ttl=21600)\n",
        src
    )
    ...
    patterns = [
        re.compile(
            r"@st\.cache_data\(ttl=21600\)\s*\ndef fetch_fred_series\(series_id\):.*?(?=@st\.cache_data\(ttl=21600\)\s*\ndef us_macro_dashboard_data\(\):)",
            re.S
        ),
        re.compile(
            r"@st\.cache_data\(ttl=21600\)\s*def fetch_fred_series\(series_id\):.*?(?=@st\.cache_data\(ttl=21600\)\s*def us_macro_dashboard_data\(\):)",
            re.S
        ),
    ]
