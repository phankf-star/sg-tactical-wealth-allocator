...
ROLE_ORDER_BY_MARKET = {
    # Broad-market implementation first.
    ...
    # Gold should prioritise defensive/physical gold vehicles before miners/satellites.
    "GOLD": {"DEFENSIVE": 0, "CORE": 1, "SATELLITE": 2, "THEMATIC": 3},

    # Bitcoin products are satellite by design.
...
def load_config():
    if not CONFIG_PATH.exists():
        return {
            ...
            "ranking_method": "role_priority_then_implementation_fit_score",
...
def yahoo_chart_fetch(symbol):
    ...
    - latest price
    - latest volume
    - average volume over available 3-month daily range

    Missing data is not fatal. The scoring function assigns a lower
    data-availability score instead of breaking the ETF master refresh.
    """
...
def percentile_score(value, values):
    """
    Convert value into a 0-100 peer-group percentile score.
    Missing values receive a low-but-not-zero score.
    """
...
def expense_score(expense_ratio):
    ...
    Missing/zero expense ratio receives neutral score because some ETF rows
    intentionally start with blank manual enrichment fields.
    """
...
def market_fit_score(row):
    """
    Manual relevance layer.

    This remains intentionally light-touch:
    - Core broad-market ETFs score higher.
    - Defensive ETFs score well where role indicates preservation/hedge use.
    - Satellite ETFs are still valid, but should not automatically outrank
      broad implementation vehicles in normal equity markets.
    """
    ...
    if asset_class in {"equity", "bond", "commodity", "crypto", "reit"}:
        score += 2
...
def main():
    ...
    for row in rows:
        ...
        if active and status == "ACTIVE":
            ...
            # Use turnover as average-volume proxy for now.
            # Later can split into avg_volume / value_traded if needed.
            if avg_volume > 0:
    ...
    print("[OK] Ranking method: role_priority then implementation_fit_score within market")
